...
| **Drift** | Animate + Char. Animator | Timeline + layers + keyframes + transforms + puppet rig (bones, IK/FABRIK, spring dynamics) + scenes + frame labels + symbol library + swap sets + mesh warp + bone weights + easing curves + audio sync + behaviors + 3D layer transforms + camera viewport + mocap import + publishing + stage settings + scripting + facial capture + AI motion stubs + advanced tweening + beat sync | ~70% | Real ONNX motion gen; webcam live capture; canvas rendering |
| **Tone** | Logic Pro / GarageBand / Ableton | Project + tracks + clips + MIDI notes/CC + piano roll + mixer + transport + AI stubs + undo/redo + automation + tempo map + scenes + plugins + MIDI control + audio engine + recording (punch-in/out, takes) + step sequencer + score view + bus routing + beat detection + clip launching + chord tools + freeze/stems + MIDI routing + virtual instruments + arpeggiator | ~70% | CPAL real-time playback; ONNX inference; VST/AU hosting |
