...
| **Drift** | Animate + Char. Animator | ~30% | Timeline + layers + keyframes + transforms + puppet rig (bones, IK/FABRIK, spring dynamics) + scenes + frame labels + symbol library + swap sets + grid/ruler config + vector paths + GPUI panels (toolbar, layers, timeline, AI panel) + welcome screen + export config + state machines | [PLAN.md](apps/drift/PLAN.md) |
| **Tone** | Logic Pro / GarageBand | ~20% | Project model + tracks + clips + MIDI notes/CC + piano roll + mixer channels + transport + AI generation stubs + bounce config + undo/redo history + MIDI editing ops (select/copy/paste/nudge/quantize/transpose) + clip ops (split/duplicate/trim/color) + track groups + insert effects chain + mixer sends/PFL/phase/stereo width + loop region state + GPUI panels (toolbar, tracks, piano roll, timeline) + welcome screen | [PLAN.md](apps/tone/PLAN.md) |
