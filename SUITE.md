...
| **Drift** | Animate + Char. Animator | ~70% | Timeline + layers + keyframes + transforms + puppet rig (bones, IK/FABRIK, spring dynamics) + scenes + frame labels + symbol library + swap sets + grid/ruler config + vector paths + state machines + mesh warp + bone weights + easing curves + audio sync + behaviors + 3D layer transforms + camera viewport + mocap import + publishing/export queue + stage settings + scripting + facial capture + AI motion (ONNX stubs) + advanced tweening + beat sync | [PLAN.md](apps/drift/PLAN.md) |
| **Tone** | Logic Pro / GarageBand / Ableton | ~70% | Project model + tracks + clips + MIDI notes/CC + piano roll + mixer channels + transport + AI generation stubs + undo/redo + MIDI editing + clip ops + track groups + insert effects + automation lanes + tempo map + scenes (session view) + plugins + MIDI control + audio engine config + recording (punch-in/out, takes) + step sequencer + score view + bus routing + beat detection + clip launching + chord tools + track freeze/stems + MIDI routing + virtual instruments + arpeggiator | [PLAN.md](apps/tone/PLAN.md) |
