...
pub enum Effect {
    ...
    /// 3D extrude: the shape is drawn offset by `depth` pixels at `angle_deg`
    /// in `color`, then the original is drawn on top. Simulates Illustrator's
    /// Extrude & Bevel with a simple shadow-extrusion approach.
    Extrude3D {
        depth: f32,
        angle_deg: f32,
        color: [f32; 4],
    },
    /// Outer glow: a halo of light emanating from the shape boundary.
    Glow {
        radius: f32,
        color: [f32; 4],
        opacity: f32,
...
impl Effect {
    ...
    /// A default 3D Extrude.
    pub fn extrude_3d() -> Self {
        Effect::Extrude3D {
            depth: 10.0,
            angle_deg: 45.0,
    ...
    pub fn label(&self) -> &'static str {
        match self {
            ...
            Effect::Extrude3D { .. } => "3D Extrude",
            Effect::Glow { .. } => "Outer Glow",
        }
    }
    ...
    pub fn is_active(&self) -> bool {
        match self {
            ...
            Effect::Extrude3D { depth, color, .. } => *depth > 0.0 && color[3] > 0.0,
            Effect::Glow { radius, color, opacity } => *radius > 0.0 && (color[3] * opacity) > 0.0,
        }
    }
    ...
    pub fn bounds_pad(&self) -> f32 {
        match self {
            ...
            Effect::Extrude3D { depth, .. } => *depth,
            Effect::Glow { radius, .. } => radius.abs() * 3.0,
        }
    }
}
