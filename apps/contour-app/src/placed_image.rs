...
impl PlacedImage {
    ...
    /// The placement transform as an SVG `transform="matrix(a b c d e f)"`
    /// attribute value. An `<image x="0" y="0" width="w" height="h">` element
    /// drawn under this matrix lands exactly where the placed image draws on the
    /// canvas, because the same [`Affine`] maps the natural rect `[0,0,w,h]` to
    /// document space in both places. Identity emits no string (the element is
    /// already in place). Numbers are emitted with up to 6 decimals, trailing
    /// zeros trimmed, so a pure translate/scale stays compact.
    pub fn svg_transform(&self) -> Option<String> {
        let t = &self.transform;
        if t.is_identity() {
            return None;
        }
...
mod tests {
    ...
    /// The SVG transform string is the placement matrix `matrix(a b c d e f)`,
    /// with identity emitting nothing (the element is already in place).
    #[test]
    fn svg_transform_maps_the_placement_matrix() {
        // Identity placement (top-left at origin, unit scale) → no transform.
        let mut img = PlacedImage::new(0, "a", embedded(10, 10), 0.0, 0.0);
        img.transform = Affine::IDENTITY;
        assert_eq!(img.svg_transform(), None, "identity emits nothing");

        // Translate to (10, 20).
        img.transform = Affine::translate(10.0, 20.0);
        assert_eq!(img.svg_transform().unwrap(), "matrix(1 0 0 1 10 20)");

        // Scale ×2/×3 about origin then translate: matrix(2 0 0 3 10 20).
        img.transform = Affine::scale(2.0, 3.0).then(Affine::translate(10.0, 20.0));
        assert_eq!(img.svg_transform().unwrap(), "matrix(2 0 0 3 10 20)");
    }
