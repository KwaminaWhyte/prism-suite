...
[Pigment](https://github.com/KwaminaWhyte/prism-suite), the raster editor). **Goal: reach ≥85% of Adobe Illustrator's real-world
...
> Companion docs: [RESEARCH.md](./RESEARCH.md) (cited findings + crate matrix), [README.md](./README.md) (current build), [SUITE.md](https://github.com/KwaminaWhyte/prism-suite/blob/main/SUITE.md) (four-app vision + interop). This PLAN expands the README's v0 scaffold into a parity roadmap; the README still tracks what runs *today*.
...
- The suite already gives us a document/color/geometry foundation (`prism-core`, `prism-color`) and a
  proven GPUI host pattern shared with Pigment (`pigment-gpui`).
...
Grounded in `contour/crates/contour-app/src/` (shared engine) and `contour-gpui/src/` (GPUI host):

- **Document model** (`document/`) — ordered `Vec<Shape>`; `Shape = Rect | Ellipse | Line | Path | Text | Compound`.
  `Path` carries real cubic-Bézier **handles**, open/closed, fill/stroke/width, visibility, layer metadata.
  `kurbo` `BezPath` build + flatten + tight bounds; robust hit-testing; translate + affine.
  Legacy-JSON-compatible (`#[serde(default)]`).
- **Tools** — Select, Direct-Select, Rectangle, Ellipse, Line, Polygon, Star, Pen (Bézier),
  Type, Eyedropper, Artboard, Shape Builder, Knife, Width, Blend, Perspective Grid, Live Paint,
  Perspective Distort, Graph (stub).
- **Canvas** (`canvas_host.rs`) — CPU raster bridge: `tiny-skia` → BGRA8 → GPUI `RenderImage`;
  artboard overlay, cursor-anchored zoom, drag pan, selection ring.
- **Pathfinder** (`boolean.rs`) — all ten ops on `i_overlay` with even-odd/non-zero fill rules,
  compound paths, Shape Builder.
- **Inspector** — fill/stroke/opacity, gradient editor, appearance stack, text params, live shapes,
  align & distribute, symbols, placed images, artboards, recolor artwork.
- **Export** (`export.rs`) — SVG, PNG, PDF via `lopdf`; AI/EPS import; symbols baked via
  `Document::flattened_for_export`.
...
| Tessellation | `lyon` 1.0 | Fill/stroke → GPU triangle mesh for native GPUI vector rendering (Phase 6). |
| Boolean / pathfinder | `i_overlay` 1.x | Union/intersect/diff/xor, fill rules. Already in use; extend to the full pathfinder set. |
| App shell + UI | `gpui` 0.2.2 | GPU-accelerated native UI (Zed's framework). `contour-gpui` is the sole shipping binary. |
...
## 3a. GPUI UI migration (complete — on `main`)

Contour's UI migrated off eframe/egui onto **[GPUI](https://www.gpui.rs/)** (Zed's
GPU-accelerated UI framework), mirroring the path proven in the sibling **Pigment** app.
`contour-gpui` is now the sole shipping binary; the eframe/egui shell has been removed.

**Why GPUI.** GPUI gives native-quality, GPU-accelerated chrome (retained elements, real layout,
crisp text) without egui's immediate-mode redraw tax. The migration was incremental — a second
binary over the same engine — so panels ported one at a time behind a stable seam.

### Architecture (landed — Phase 1+2 starter)

- **`contour-app` is a library.** `lib.rs` exposes the module tree (document model, rasterizer, etc.)
  so `contour-gpui` reuses the *same engine* — no fork. The split is purely structural
  (every module uses `crate::…`); the GPUI host is a thin CPU bridge over this shared engine.
- **One shared state, one mutation choke point.** `app_state::App` owns everything panels read or
  mutate (document, active tool, selection, preview host). Panels never mutate it — they emit an
  `Action`, and the root view routes it through `App::apply`, the **single** place state changes (and
  the host is marked dirty when pixels change). A new interaction = one `Action` variant + one `apply`
  arm, so panels port in parallel without colliding.
- **CPU preview bridge (no GPU compositor).** Unlike Pigment (which bridges a wgpu compositor), Contour
  rasterizes vectors CPU-side via tiny-skia, so the host reuses the export raster path:
  `export::to_rgba8_artboard` (additive; shares the PNG exporter's pixmap) yields the active artboard's
  raw straight-RGBA8 pixels → `canvas_host` converts RGBA8 → BGRA8 (GPUI `RenderImage` byte order),
  wraps it as a `RenderImage`, and **dirty-caches** it so it only re-rasterizes on change.
- **Live chrome + the first real panel.** Top toolbar, left tools strip (active tool highlighted,
  click → `SetTool`), center artboard preview, right dock. The **Layers panel** is wired end-to-end:
  per-shape rows with a visibility toggle (re-rasterizes) and row-select, the canonical `Action`
  round-trip documented verbatim in `panels/mod.rs`.

### Migration roadmap

- **Phase 1+2 (done):** host shell + shared-state seam + CPU preview bridge + Layers panel. ✔
- **Phase 3 — port the panels:** Properties / Appearance (fills, strokes, opacity), Swatches, Stroke,
  Artboards, Align, History. Each follows the `panels/mod.rs` convention (read `&App`, emit `Action`).
  *(Started: an **Inspector** panel edits the selected shape's fill colour, stroke colour, stroke
  width, and opacity (fill alpha) via `Action::SetFillColor/SetStrokeColor/SetStrokeWidth/SetOpacity`
  → re-rasterize. **History is now wired** — `contour-app`'s shared snapshot `History` drives
  undo/redo (Cmd+Z / Cmd+Shift+Z + toolbar buttons) over create/delete/appearance/pen/node edits,
  with discrete checkpoints and coalesced drags. Still open: the full stacked Appearance, Swatches,
  Stroke style, Align, and a dedicated History panel.)*
- **Phase 4 — interactive canvas:** hit-testing, selection, and the draw/edit tools wired through
  `Action`s; replace the static preview with an interactive surface (pan/zoom, marquee, direct-select).
  *(Done so far: **click-to-select** (`Action::HitTestSelect`, reusing `Shape::hit`), **pan/zoom**,
  **Rect/Ellipse** drag-create, the **Pen tool** (click-place anchors + drag bezier handles, close on
  first-anchor/Enter, Escape to cancel — commits a `Shape::Path`), and **Direct-Select** node editing
  (grab + drag anchors / tangent knobs of the selected path, handles prioritised). The **Line /
  Polygon / Star** tools now drag-create (Line from endpoints; Polygon/Star centre-out into a live
  `Shape::Path` carrying `LiveShape::{Polygon,Star}` params via `live_shape`). The **Type tool** places
  a `Shape::Text` on click and edits its string live (re-laying-out glyphs through
  `contour_app::text::layout`; Backspace/Enter/Esc; empty text discarded; one coalesced undo entry).
  **Boolean / pathfinder** ops (Unite / Minus / Intersect / Exclude) run on a primary + Shift-clicked
  secondary selection through `contour_app::boolean::apply` (`i_overlay`), replacing both operands with
  the styled result and selecting it (undoable; toolbar buttons greyed until two distinct shapes are
  picked). All draw/edit gestures are undoable. **Marquee + multi-select** now ship: the selection is a
  canonical `Vec<usize>` (primary = last, secondary = previous — boolean front/back and every prior
  single-select path unchanged); an empty-canvas drag rubber-bands a marquee (`MarqueeSelect`, bounds-
  intersection) and **Shift+click** toggles membership (`AddToSelection`), with per-member + union-ring
  overlays, multi-delete in one undo step, and the set driving the inspector / boolean / align group. The
  **live-shape + text inspector controls** also ship: Polygon/Star sides-points/radius/inner-ratio
  steppers (`SetLiveShape` → `set_live_shape`) and text size/align/font controls (`SetTextSize/Align/Font`
  → `set_text_params`), each one undo step. **Align & distribute** is wired into the toolbar over the
  multi-selection (`align::{align_deltas, distribute_deltas, union_bounds}` → `translate`): align left/
  center-H/right/top/center-V/bottom (≥2) and distribute H/V centers (≥3), greyed until the set is big
  enough. Still open: multi-anchor node selection, compound-path editing, add/delete-anchor on a segment,
  align-to-artboard / distribute-spacing / per-edge distribute in the host UI, and the remaining tools
  (Artboard/Eyedropper/ShapeBuilder). **Wave 8 (done):** symbols, gradient-stop editor, image-trace UI,
  recolor panel, artboard tool. **Wave 9 (done):** eyedropper pixel sampling (`CanvasHost::sample_pixel`,
  `PickColor`, auto-revert); shape builder stub; text-on-path inspector; SVG export (plain-string builder,
  toolbar button + 3-s status toast); SVG import (tag-scan parser, no deps); keyboard shortcuts (V/A/P/E/R/T/I
  tool keys, Cmd+G/Shift+G group/ungroup, Cmd+[/] layer order); 38 tests green.
  **Wave 11 (done):** gradient-fill inspector; Knife tool. **Wave 12 (done):** on-canvas transform
  handles (8 scale squares, MoveSelected drag); SVG line/group/named-color import.
  **Batch 3 (done):** Live Paint Bucket (`Tool::LivePaint`, `ApplyLivePaint`); Type on Path (`PlaceTextOnPath`, `text_on_path` module stub); Perspective Distort (`Tool::PerspectiveDistort`, `SetPerspectiveDistort`, `perspective` module stub with `homography`/`warp_pixels`); Recolor Artwork HSL (`shift_hsl` in recolor.rs, `RecolorArtworkHSL` action, HSL panel state); Distribute gap equalize (HorizontalGap/VerticalGap toolbar buttons), Align to Artboard (`AlignToArtboard` action + 6 accent toolbar buttons).*
  **Batch 4 (done):** Envelope Distort (`Tool::Envelope`, `contour_app::envelope::EnvelopeMesh`, `MakeEnvelopeWithMesh`/`MoveEnvelopePoint`/`ReleaseEnvelope`); Outer Glow live effect (`Effect::Glow`, `AppearanceAddGlow`, `ToggleEffect`/`RemoveEffect`/`ReorderEffect`); Group blend modes (`SetGroupBlendMode`/`SetGroupOpacity`, `App::group_blend_modes`/`group_opacities`); Pathfinder improvements (`OutlineStroke` → `SubPath::ring` compound, `ExpandAppearance` stub); Graphic Styles panel (`panels/graphic_styles.rs`, seeded with 5 default styles); toolbar Outline/Expand buttons.*
  **Batch 5 (done):** finished two long-standing stubs + three new vector-authoring ops, all end-to-end. **Type on a Path — real arc-length rendering** (`text_on_path::layout_on_path` walks the spine by `kurbo` arclen/inv_arclen/deriv, rigid-transforms each glyph onto the tangent + path-normal offset, **bakes the warped glyphs into the text's existing glyph cache** so canvas/SVG/PNG render with no per-surface code; `TextOnPathParams { offset, start, flip }`, `App::text_on_path_params`/`relayout_text_on_path`, `SetTextOnPathParams`, inspector Offset/Start/Flip controls). **Perspective Distort — real homography** (closed-form unit-square→quad solve in `perspective::homography` + `warp_points`/`warp_handles`; `ApplyPerspectiveDistort` warps anchors+handles of a Path/Compound through `H`, one-click trapezoid or on-canvas corners; `warp_shape_perspective` host helper). **Variable-width stroke outline** (`pathedit::variable_width_outline` arc-length-interpolates `StrokeStyle::width_profile` into a tapered filled band; `OutlineWidthProfile` — closes the Phase 3 width-profiles item). **Roughen distort** (`pathedit::roughen` deterministic subdivide+jitter; `RoughenPath`). **Gradient Mesh object** (`mesh_gradient::seed_grid` seeds the 4×4 editable grid from the selection's bounds+fill into the existing Wave-15 overlay compositor; `MakeMeshGradient`/`ClearMeshGradient`). Toolbar gains Width▶ / Roughen / Mesh / Persp▶ buttons. Also repaired the `cargo test -p contour-app` build (60 carried-over additive-field literal sites). 559 tests green.*
- **Phase 5 — prism-ui integration (Wave 10):** ✅ DONE — prism-ui design tokens replace all
  hardcoded rgb constants; text-label buttons replaced with icon equivalents. *(Gradient fill rendering
  in canvas_host and shape builder geometry are separate tasks — deferred to a later wave.)*
- **Phase 6 — native vector rendering:** draw paths as GPUI/lyon geometry instead of a CPU-rasterized
  `RenderImage`, eliminating the readback for crisp, resolution-independent on-screen vectors.
- **Phase 7 — retire egui:** ✅ DONE — `contour-gpui` is the sole binary; the eframe shell has been
...
- [x] App shell (GPUI), Prism theme/icons, artboard canvas, pan + cursor-anchored zoom
...
- [x] Arrange: bring-to-front/back, group/ungroup, lock, paste-in-place/in-front *(Done: a pure, unit-tested `arrange` module computes a stacking-order permutation for the four z-order ops — **Bring to Front / Bring Forward / Send Backward / Send to Back** — applied to the selection (relative order preserved, no-ops clamp at the ends) and a `group` module tags shapes with an additive `group: Option<u64>` so **Group / Ungroup** collect/release a selection as one unit. Wired into an **Object ▸ Arrange** menu + inspector row + Layers-panel reorder, with Illustrator keys **Cmd/Ctrl+]/[** (Shift = to front/back) and **Cmd/Ctrl+G / +Shift+G**; each is a single undo step. `lock` ships via the Layers panel (`Shape::selectable()` gate) and **paste-in-place/-front/-back** via the clipboard. No model change beyond the additive group tag (`#[serde(default)]`).)*
...
- [-] **Live shapes** (M): editable rounded-rect corners, polygon/star sides & radius, arc/spiral/grid; live corner widget *(Done: **Polygon** (N sides + radius) and **Star** (N points + radius + inner ratio) parametric live shapes — pure `liveshape` generator, additive `Path::live` serde, drag-from-centre tools with preview, a Live Shape inspector that regenerates the outline about the shape's centre, and Illustrator-style demote-to-path on direct anchor edit. Still open: editable rounded-rect corners, arc / spiral / grid, the on-canvas live corner widget.)*
- [-] **More creation tools** (M): polygon, star, rounded-rect, arc, spiral, rectangular/polar grid, **Pencil** (freehand fit), **Curvature** tool, **Blob brush**, line/segment *(Done: **polygon** + **star** tools (drag from centre → radius). Still open: rounded-rect, arc, spiral, rectangular/polar grid, Pencil, Curvature, Blob brush, line/segment.)*
- [-] **Path editing** (M): join, average, simplify (reduce anchors), smooth, **scissors**, **knife**, reshape, offset path, outline stroke *(Done: **Simplify** (Douglas–Peucker anchor reduction on the flattened outline, adjustable tolerance, endpoints + closed-ring floor preserved, idempotent on a minimal path), **Offset Path** (signed miter-join polygon offset — `+` outsets / `−` insets a closed path winding-independently, open paths shift to one side, `0` = identity), and **Outline Stroke** (convert a path's stroke to a filled outline: ±half-width band → fill = former stroke colour, no stroke; open path → one closed band, closed path → even-odd annulus `Compound`; no-op when unstroked) under a new **`Object ▸ Path`** menu. All are pure helpers in a new `pathedit` module (`simplify` / `offset_path` / `outline_stroke`); the UI demotes the selection to a plain corner path first (`Shape::to_path`, dropping `live` params), per-sub-contour for a Compound, as one undo step. Unit-tested. **Still open:** join open endpoints, average, smooth, scissors, knife, reshape; round-join offset / outline (outline uses miter + butt caps today); refitting bezier handles instead of emitting corner anchors.)*
...
- [-] **Type** (L): **point type** + **convert to outlines** + **font family selection** — *landed* (Type tool `T`; click-to-place + inline edit with backspace/enter/esc; font-size + left/centre/right alignment; multi-line; real glyph outlines extracted via `ttf-parser`, cached on `Shape::Text` and composing / exporting like a compound path through canvas + SVG + PNG; `Object ▸ Type ▸ Create Outlines` → editable `Compound`; `.contour` round-trip). **Font selection done:** a pure `fonts` module enumerates installed system families via `fontdb` (same `ttf-parser` core) with the bundled **Ubuntu** face as the guaranteed default/fallback; a **Font** dropdown in the Type inspector re-extracts outlines from the chosen face (canvas + SVG + PNG); the family is persisted per text object as additive `font_family` (`#[serde(default)]` → bundled), with unknown/missing families resolving gracefully to the bundled face and loaded faces cached. A **font/size change re-lays-out about the object's existing `origin`**, so a placed (and moved) text object keeps its position — guarded by regression tests. *Open:* **area type**, **type-on-path**, threaded/overflow text, character + paragraph panels, **weight / style sub-selection** + OpenType shaping / kerning + **variable fonts**, glyphs panel, text wrap, tabs, character/paragraph **styles**.
- [-] **Color system** (M, shared): swatches, **global** + **spot** colors, color groups, CMYK/RGB/spot modes via `prism-color`, **Recolor Artwork** (palette remap/harmony rules), eyedropper, live-paint bucket. *(Done: a document **Swatches** palette — a pure, fully unit-tested `swatches` module (`Swatch { id, name, color, global }` + `Swatches`) carried on the document as additive `swatches: Swatches` (`#[serde(default)]` → starter palette, so pre-swatches `.contour` files round-trip and shapes keep their literal colours). A **Swatches panel** (left dock) builds the palette from the current fill (`+`), shows a chip grid (active-fill ringed, **global** swatches corner-dotted), and **applies** a swatch to the selection's **fill** (click) or **stroke** (shift-click) as one undo step; an inline editor renames, recolours, toggles **global**, and deletes the selected swatch. A **global** swatch propagates a recolour to all artwork painted with it: `Swatches::recolor` is the pure resolver (returns the `(old,new)` pair only for a global edit; a non-global edit is a one-time copy that touches nothing but the swatch), and `Document::remap_color` is the pure artwork walk that remaps fills / strokes / gradient stops / appearance-stack paints and reports the affected count. **Eyedropper** tool (`I`) samples the topmost shape's paint under the cursor into the active fill/stroke defaults and applies it to the selection (Alt = pick-up-only). Guarded by unit tests: starter-palette uniqueness, dedup-by-colour (with picker tolerance), unique-name/rename discipline, global recolour returns the pair / non-global returns `None` but still edits, document `remap_color` across fills+strokes+gradients with count, serde round-trip incl. legacy (no-swatches → starter, literal colours unchanged), and eyedropper sample/apply. **Recolor Artwork** (Object ▸ Recolor Artwork…): a pure `recolor` module — `extract_palette` (the selection's distinct used colours across fills/strokes/gradient stops/appearance paints), `reduce_to_n` (deterministic seedless k-means in luminance-weighted sRGB), `harmony` (Complementary/Analogous/Triadic/Tetradic via HSL hue rotation), and `recolor` (index-paired (old,new) remap applied through `Document::remap_color` as one undo step) — driven by a dialog showing current vs editable new palette with reduce-to-N + harmony controls and a live remap count. **Still open:** **spot** colours + colour groups, CMYK/spot colour modes via `prism-color`, and the **live-paint** bucket.)*
- [-] **Symbols** (M): symbol library, instances, edit-master propagation, symbol sprayer/shifter/sizer set. *(Done: a pure, fully unit-tested `symbols` module — `Symbol { id, name, shapes }` (master = a shape set), `SymbolInstance { id, symbol, transform: Affine }` (a reference + its own placement), and an ordered, name-unique, id-stable `Symbols` collection — carried on the document as additive `symbols: Symbols` (`#[serde(default)]` → empty; pre-symbols `.contour` round-trips). An instance carries **no geometry**: it is **resolved** on demand (`Symbols::resolve`) by cloning the **live** master's shapes and applying the instance transform, so **edit-master propagation** is automatic — editing a master (move/recolour/reshape) changes every instance with no per-instance fix-up. A **Symbols panel** (right inspector): **Create from selection** (captures a master, replaces the selection in place with an identity instance), **Place** more instances, **Update master** from the selection (all instances follow), per-symbol rename/delete (delete cascades to its instances); a selected instance shows a canvas ring and can be nudged/scaled/rotated numerically, **Break link**-ed to editable shapes, or deleted — each a labelled undo step. Instances render resolved on canvas and are baked into **SVG/PNG export** (`Document::flattened_for_export`); `Affine` is now serializable. Guarded by unit tests: define-from-N, two instances through their own transforms, master-edit propagation (geometry+colour), break/delete, symbol-delete cascade, dangling-instance safety, id non-reuse, serde round-trip incl. legacy, determinism. **Still open:** the symbol **sprayer / shifter / sizer** tool set, and on-canvas direct drag/scale/rotate of an instance (panel-driven numeric edits today).)*
- [ ] **Brushes** (L): calligraphic, art, scatter, pattern, bristle brushes along paths; brush library
- [x] **Image trace** (M): `vtracer` raster→vector with presets (B/W, color), threshold/path-fitting controls, expand to paths. *(Done: `File ▸ Image Trace…` and an `Object ▸ Image Trace` controls submenu pick a raster (PNG/JPEG/BMP/GIF via `image`), trace it with `vtracer` 0.6, and add one filled closed `Shape::Path` per traced contour — grouped, on top, one undo step. Two **mode presets** — B/W (binary, with a luminance **threshold**) and **Color** (with **color precision**) — plus **filter speckle**, **corner threshold**, **path precision** knobs at vtracer's defaults. The vtracer SVG-path-`d` → anchor/handle conversion (`trace::svg_path_to_contours`) is a pure, testable `M`/`L`/`C`/`Z` parser. 8 unit tests: parsing/handle correctness, multi-subpath, neg/decimal coords, black-square bbox, B/W vs color both produce grouped shapes, determinism, empty-input error.)* Sketch / silhouette presets deferred.
- [-] **Place / link images** (M): embed or link raster; clipping mask; crop image; round-trip placed `.pigment` via suite interop *(Done: a pure, fully unit-tested `placed_image` module — `PlacedImage { id, name, source, transform, clip }` where `ImageSource` is **`Embedded`** (RGBA8 pixels stored in the document) or **`Linked`** (path + cached natural size, pixels re-read from disk), carried on the document as additive `placed_images` (`#[serde(default)]` → empty; pre-Place `.contour` round-trips). The placement transform is applied to the image's natural pixel rect; an optional **clipping mask** (a closed document-point ring) bounds where it draws — all pure `f32`: `corners` / `drawn_bounds` / `clip_bounds` (image ∩ clip) / `clip_contains` / `hit`. **File ▸ Place Image ▸ Embed… / Link…** (`rfd` pick, `image` decode, centred on the active artboard); renders on canvas as a textured quad (position/scale/rotation) clipped to the clip bounds, linked pixels cached per session so paint never hits disk; **Placed Images inspector** — place, select, numeric move/scale/rotate, **Make clipping mask** from the topmost selected path (mask consumed) + **Release clip**, **Relink/refresh**, delete (each one undo step); canvas click-selects the topmost hit image. Guarded by unit tests: drawn rect reflects position+scale; clip limits the drawn region to the path bounds (non-overlapping clip excludes it); embed stores pixels / link stores path; id non-reuse; serde round-trip incl. legacy + determinism; document-level round-trip + legacy load. **Placed images now bake into SVG/PNG export** (in canvas z-order, honouring transform + clip): PNG composites each image's pixels through its placement `Affine` as a tiny-skia `Pattern` quad clipped by a `Mask` (linked sources re-read from disk); SVG emits an `<image>` with a `matrix(...)` transform — Embedded → base64 PNG `data:` URI, Linked → `href`/`xlink:href` path — and a clip ring as a `userSpaceOnUse` `<clipPath>`. Additive: a document with none exports byte-identical SVG (xlink ns only added with an `<image>`) + PNG; pure `svg_transform` / href-by-source / base64 + bake tests. **Still open:** **crop** the placed image, live mtime polling of links (relink is explicit), and the placed-`.pigment` **suite-interop** round-trip.)*
...
- [x] **EPS / AI bridge** (M): best-effort text-format `.ai`/`.eps` (CS4 and earlier) import via `ai_eps::import` (Wave 14); SVG remains the recommended interchange path for newer AI files.  EPS export deferred.
...
- [ ] **Performance** (L): move large-doc rendering from the CPU-raster bridge to a `lyon`→GPUI native vector path; spatial index (R-tree) for hit-test/selection on docs with 10k+ objects; parallel boolean/tessellation (`rayon`)
...
| Type / area / on-path / variable | full | **Point type + convert-to-outlines + font-family selection (system fonts via `fontdb`) done; area / on-path / variable fonts planned** | 5 |
...
1. **Undo** → ~~add a command stack~~ Done: `history.rs` snapshot stack, drag-coalesced, wired into every mutation.
2. **Performance at scale** → CPU-raster-bridge-per-frame won't hold 10k objects; move to native GPUI vector paths via `lyon`, add an R-tree spatial index, parallelize boolean/tessellation.
...
- [x] Symbol library — real `contour_app::symbols::Symbols`; `DefineSymbol` / `PlaceSymbol2` / `EditSymbol2` / `DeleteSymbol`; symbols panel rewritten.
- [x] Variable font axes — `font_axes: HashMap<String,f32>` on `TextParams`; `SetFontAxis` action; wght/wdth/slnt steppers in character panel.
- [x] Named artboards — `ArtboardEntry { id, name, rect }`; `active_artboard`; `AddArtboard2` / `RemoveArtboard` / `RenameArtboard` / `SelectArtboard` / `DuplicateArtboard`; artboards panel in right dock; canvas overlay uses named entries.
- [x] Graph / chart tool stub — `Tool::Graph`; `GraphKind { Bar, Line, Pie, Scatter }`; `InsertGraph` action generates shapes from sample data.
- [x] Stroke width presets — `WidthProfilePreset { Uniform, TaperIn, TaperOut, BulgeCenter, TaperBoth }`; `ApplyWidthPreset` action; inspector "Width Presets" section.
...
- [ ] **Dockable / floating panels** (L) — drag panels out, redock, split; tear-off windows. (GPUI layout system; needs a custom dock tree.)
...
- [x] **Recolor Artwork** (M) — remap a selection's colors via a palette/harmony UI, global-color edits, reduce-to-N-colors. *(**Done:** a pure, fully unit-tested `recolor` module — `extract_palette` (the artwork's distinct used colours, covering fills/strokes/gradient stops/appearance paints), `reduce_to_n` (deterministic seedless k-means in luminance-weighted sRGB → reduced palette + per-colour cluster mapping), `harmony` (Complementary/Analogous/Triadic/Tetradic via HSL hue rotation), and `recolor` (index-paired (old,new) remap) — plus an **Object ▸ Recolor Artwork…** dialog (current vs editable new palette, reduce-to-N, harmony rule, live remap-count, Apply) that applies via the existing `Document::remap_color` artwork-walk as one undo step, scoped to the selection or whole document.)*
- [x] **Image Trace** (M) — `vtracer` raster→vector with B/W / color presets and an expand-to-paths step. *(Done — see §5. `File ▸ Image Trace…` traces a picked raster to grouped filled paths; B/W + Color modes with threshold / colour-precision / speckle / corner / path-precision controls.)*
...
- [x] **Symbols & global swatches in the UI** (M) — a Swatches panel and a Symbols panel (instances + edit-master). *(**Done:** Swatches panel (build/apply/global-edit/eyedropper — see §5 Color system) **and** the Symbols panel — create-from-selection, place instances, edit/update-master with propagation, rename/delete, instance break/delete + numeric transform — see §5 Symbols. **Open follow-up:** the symbol sprayer/shifter/sizer tools and on-canvas instance drag.)*
