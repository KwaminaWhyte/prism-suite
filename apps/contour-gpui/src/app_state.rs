...
use contour_app::align::{self, Align, Distribute};
use contour_app::appearance::{Appearance, Effect, Fill, Paint, Stroke as AppStroke};
use contour_app::boolean::{self, BoolFillRule, BoolOp};
use contour_app::document::{self, Document, Shape};
use contour_app::gradient::{Gradient, GradientKind, GradientStop};
use contour_app::history::History;
use contour_app::liveshape::LiveShape;
use contour_app::text::{TextAlign, TextParams};
use contour_app::transform::{self, Affine, Handle};
...
pub enum Action {
    ...
    SetGroupBlendMode { group_id: u64, mode: contour_app::appearance::BlendMode },
    ...
    SetTextOnPathParams {
        text_id: usize,
        params: contour_app::text_on_path::TextOnPathParams,
    },
...
pub struct App {
    ...
    pub symbol_lib: contour_app::symbols::Symbols,
    ...
    pub group_blend_modes: std::collections::HashMap<u64, contour_app::appearance::BlendMode>,
    ...
    pub text_on_path_params:
        std::collections::HashMap<usize, contour_app::text_on_path::TextOnPathParams>,
}
...
impl App {
    ...
    pub fn new() -> Self {
        ...
        {
            use contour_app::appearance::{Appearance, Effect, Fill, Stroke};
        ...
        Self {
            ...
            xform_start: (0.0, 0.0),
            symbols: Vec::new(),
            symbol_lib: contour_app::symbols::Symbols::default(),
    ...
    pub fn relayout_text_on_path(&mut self, text_id: usize) {
        ...
        let (bez, closed) = contour_app::text_on_path::spine_bezpath(&self.doc.shapes[path_id]);
        ...
        if let contour_app::document::Shape::Text { params, glyphs, .. } =
            &mut self.doc.shapes[text_id]
        {
            let warped = contour_app::text_on_path::layout_on_path(
                params,
                &bez,
                closed,
                params_on_path,
    ...
    pub fn apply(&mut self, action: Action) {
        match action {
            ...
            Action::PlaceText { x, y } => {
                ...
                let glyphs = contour_app::text::layout(&params, (x, y)).0;
            ...
            Action::AddArtboard(rect) => {
                ...
                let name = contour_app::artboard::default_name(self.artboard_entries.len());
            ...
            Action::ApplyShapeBuilder { subtract } => {
                ...
                let sel_shapes: Vec<document::Shape> = sel
                    ...
                    .collect();
                let faces = contour_app::shapebuilder::build_faces(&sel_shapes);
                if faces.is_empty() {
                    return;
                }
                let all_face_indices: Vec<usize> = (0..faces.len()).collect();
                let mode = if subtract {
                    contour_app::shapebuilder::BuildMode::Subtract
                } else {
                    contour_app::shapebuilder::BuildMode::Unite
                };
                let results = contour_app::shapebuilder::apply_build(
                    &sel_shapes,
                    &faces,
                    &all_face_indices,
                    mode,
            ...
            Action::CreateBlend { steps } => {
                if let (Some(a_idx), Some(b_idx)) = (self.selected, self.secondary) {
                    if let (Some(a), Some(b)) = (
                        ...
                        self.checkpoint();
                        let new_steps = contour_app::blend::make_steps(&a, &b, steps);
            ...
            Action::ImportAiEps(path) => {
                self.checkpoint();
                match contour_app::ai_eps::import(&path) {
                    Ok(shapes) => {
                ...
                if let Some(idx) = self.selected {
                    ...
                    if let Some(shape) = self.doc.shapes.get_mut(idx) {
                        ...
                        if let Some(ap) = shape.appearance_mut().as_mut() {
                            ap.effects.push(contour_app::appearance::Effect::extrude_3d());
                        }
                        self.host.mark_dirty();
                    }
                }
            ...
            Action::AppearanceSet3DDepth { idx, depth } => {
                if let Some(si) = self.selected {
                    if let Some(shape) = self.doc.shapes.get_mut(si) {
                        if let Some(ap) = shape.appearance_mut().as_mut() {
                            if let Some(contour_app::appearance::Effect::Extrude3D { depth: d, .. }) = ap.effects.get_mut(idx) {
                                *d = depth;
                                self.host.mark_dirty();
                            }
                        }
            ...
            Action::AppearanceSet3DAngle { idx, angle_deg } => {
                if let Some(si) = self.selected {
                    if let Some(shape) = self.doc.shapes.get_mut(si) {
                        if let Some(ap) = shape.appearance_mut().as_mut() {
                            if let Some(contour_app::appearance::Effect::Extrude3D { angle_deg: a, .. }) = ap.effects.get_mut(idx) {
                                *a = angle_deg;
                                self.host.mark_dirty();
                            }
                        }
            ...
            Action::PlaceSymbol2(sym_id) => {
                if let Some(inst_id) = self.symbol_lib.place(
                    sym_id,
                    contour_app::transform::Affine::translate(180.0, 180.0),
                ) {
                    let resolved = {
                    ...
                    for i in sel {
                        if let Some(shape) = self.doc.shapes.get_mut(i) {
                            if let Some(c) = shape.fill_color() {
                                let nc = contour_app::recolor::shift_hsl(
            ...
            Action::MakeEnvelopeWithMesh { shape_id, rows, cols } => {
                if shape_id < self.doc.shapes.len() {
                    ...
                    let mesh = contour_app::envelope::EnvelopeMesh::new(rows, cols, bbox);
            ...
            Action::AppearanceAddGlow => {
                if let Some(idx) = self.selected {
                    ...
                    if let Some(shape) = self.doc.shapes.get_mut(idx) {
                        ...
                        if let Some(ap) = shape.appearance_mut().as_mut() {
                            ap.effects.push(contour_app::appearance::Effect::glow());
                        }
                        self.host.mark_dirty();
                    }
                }
            ...
            Action::OutlineStroke(shape_id) => {
                if shape_id < self.doc.shapes.len() {
                    ...
                    if let Some(pts) = self.doc.shapes[shape_id].outline_polygon() {
                        ...
                        let outer = contour_app::stroke::offset_contour(&pts, half_w, true);
                        let inner = contour_app::stroke::offset_contour(&pts, -half_w, true);
                        use contour_app::document::{Shape, SubPath};
                        use contour_app::document::FillRule;
            ...
            Action::MakeMeshGradient => {
                if let Some(idx) = self.selected {
                    if let Some(b) = self.doc.shapes[idx].bounds() {
                        ...
                        self.mesh_points = contour_app::mesh_gradient::seed_grid(bbox, base);
                        self.host.mark_dirty();
                    }
                }
            }
                ...
                if shape_id < self.doc.shapes.len() {
                    ...
                    let centreline = match self.doc.shapes[shape_id].to_path() {
                        Shape::Path { points, handles, closed, .. } => {
                            Some(contour_app::document::flatten(&points, &handles, closed))
                        }
                        _ => None,
                    };
                    if stroke_w > 0.0 {
                        if let Some(pts) = centreline {
                            ...
                            let band = contour_app::pathedit::variable_width_outline(
            ...
            Action::RoughenPath { size, detail } => {
                ...
                if !sel.is_empty() {
                    ...
                    for i in sel {
                        ...
                        {
                            let rough = contour_app::pathedit::roughen(points, *closed, size, detail);
                            if rough.len() >= 2 {
...
fn shape_to_svg(shape: &contour_app::document::Shape) -> String {
...
fn import_svg(
    path: &std::path::Path,
) -> Result<Vec<contour_app::document::Shape>, Box<dyn std::error::Error>> {
    use contour_app::document::Shape;
    ...
    while i < content.len() {
        ...
        if tag.starts_with("<rect") {
            ...
            if !text.is_empty() {
                let params = contour_app::text::TextParams {
                    text: text.clone(),
                    font_size,
                    ..Default::default()
                };
...
fn rand_group_id(_doc: &contour_app::document::Document) -> u64 {
...
fn warp_shape_perspective(
    ...
    use contour_app::document::{Shape as S, SubPath};
    use contour_app::perspective::{warp_handles, warp_points};
    match path {
        S::Path {
            points,
            handles,
