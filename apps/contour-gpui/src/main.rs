//! Contour — GPUI host (coexists with the eframe/egui `contour` binary).
//!
//! Architecture, honest from day one: GPUI paints the chrome (top toolbar, left
//! tools strip, right dock of panels), while the document pixels come from
//! `CanvasHost`, which drives Contour's REAL CPU vector rasterizer (tiny-skia,
//! via `contour_app::export`) and bridges the result in as a `RenderImage`.
//! Contour has no GPU compositor, so — unlike Pigment — this host's bridge is
//! pure CPU readback; drawing vectors as native GPUI shapes is a later pass.
//!
//! The integration backbone (mirrors `pigment-gpui`):
//! - [`app_state::App`] owns everything panels read/mutate (host, doc, tool,
//!   selection, view transform) and exposes the single mutation choke point
//!   `App::apply`.
//! - Panels live in [`panels`] as `render(app: &App, cx: &mut Context<Contour>)`
//!   functions and emit [`app_state::Action`]s via `cx.listener` →
//!   `root.app.apply(...)`. See `panels/mod.rs` for the verbatim convention.
//! - This root view (`Contour`) holds the `App`, lays out the chrome around the
//!   bridged document preview, and owns the canvas mouse/scroll interaction:
//!   click-select, drag-create (Rect / Ellipse), and pan + zoom.
...
use contour_app::transform::Handle;
use gpui::{
