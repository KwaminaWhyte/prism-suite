...
impl App {
    ...
    pub(super) fn apply_batch13(&mut self, action: Action) {
        match action {
            ...
            // Not a Batch-13 action: chain to the Batch-12 handler (which has its
            // own terminal catch-all). This keeps both waves' tails reachable.
            other => self.apply_batch12(other),
        }
    }
