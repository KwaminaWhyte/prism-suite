...
pub struct App {
    ...
    // --- Width tool ---
    /// Per-shape variable-width stroke profiles, keyed by paint-order index. A
    /// shape absent from the map strokes uniformly at its `stroke_w`; the Width
    /// tool actions (`WidthPoint*` / `WidthApplyPreset`) create / edit a profile,
