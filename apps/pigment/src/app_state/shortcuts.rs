...
mod tests {
    ...
    fn conflict_detected() {
        ...
        // Rebinding a command to a chord that ONLY it holds is not a conflict.
        // (Use a clean map: above, `custom.b` also holds Cmd+K, so on `m` the
        // chord legitimately collides for everyone.)
        let mut solo = ShortcutMap::defaults();
        let own = KeyChord::parse("Cmd+K").unwrap();
        solo.remap("custom.a", own.clone());
        assert!(!solo.would_conflict("custom.a", &own)); // same command = no conflict
    }
