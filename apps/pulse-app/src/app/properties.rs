            ...
            .show_inside(root, |ui| {
                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        ...
                        // Effect stack (color-correction passes). Nulls and Guides draw
                        // nothing, so an effect stack on them would do nothing — hide it.
                        if !matches!(self.comp.layers[idx].kind, LayerKind::Null | LayerKind::Guide) {
    ...
    fn source_duration_for(&self, idx: usize, comp_fps: f32) -> Option<f32> {
        ...
        match layer.kind {
            LayerKind::Footage => match layer.footage.source.as_ref()? {
                ...
                FootageSource::Video { fps, frame_count, .. } => {
                    Some(*frame_count as f32 / fps.max(0.1) as f32)
                }
    ...
    fn track_row(
        ...
        let resolved = get(layer).sample_expr(
            ...
            ExprCtx {
                ...
                index: id.1,
                width: self.comp.width as f32,
                height: self.comp.height as f32,
            },
        );
        let layer = &mut self.comp.layers[id.1];
    ...
    match effect {
        ...
        Effect::MotionBlur {
            ...
            ui.horizontal(|ui| {
                ui.add_space(8.0);
                ui.label("Samples");
                let mut s = *samples as f32;
                ui.add(egui::Slider::new(&mut s, 1.0..=32.0).integer());
                *samples = s as u32;
            });
            slider(ui, "Shutter Angle", shutter_angle, 0.0, 360.0);
        }
    }
}
...
fn stylize_effect_params(ui: &mut egui::Ui, _idx: usize, _ei: usize, effect: &mut StylizeEffect) {
    match effect {
        ...
        StylizeEffect::Stroke { width, color, .. } => {
            ui.horizontal(|ui| {
                ui.add_space(8.0);
                ui.label("Width");
                ui.add(egui::Slider::new(width, 0.0..=50.0));
            });
            ui.horizontal(|ui| {
                ui.add_space(8.0);
                ui.label("Color");
        ...
        StylizeEffect::Posterize { levels } => {
            ui.horizontal(|ui| {
                ui.add_space(8.0);
                ui.label("Levels");
                ui.add(egui::Slider::new(levels, 2..=32));
            });
        }
        ...
        StylizeEffect::Threshold { level } => {
            ui.horizontal(|ui| {
                ui.add_space(8.0);
                ui.label("Level");
                ui.add(egui::Slider::new(level, 0.0..=1.0));
            });
        }
    }
}
...
fn distort_effect_params(ui: &mut egui::Ui, idx: usize, ei: usize, effect: &mut DistortEffect) {
    ...
    match effect {
        ...
        DistortEffect::DisplacementMap { map_layer, scale_x, scale_y } => {
            ui.horizontal(|ui| {
                ui.label("Map Layer");
                ui.add(egui::DragValue::new(map_layer));
            });
            slider(ui, "Scale X", scale_x, -1.0, 1.0, "");
            slider(ui, "Scale Y", scale_y, -1.0, 1.0, "");
        }
    }
}
