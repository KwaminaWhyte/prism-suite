...
//! else.
//!
//! Real **video decode (FFmpeg)** is now supported via the `prism-media` crate:
//! a [`FootageSource::Video`] variant stores the probed metadata and the
//! [`FrameCache`] video cache decodes individual frames on demand through
//! `prism_media::decode_frame_at` (the ffmpeg CLI shim). Missing ffmpeg degrades
//! gracefully to black (logged, no panic).
...
pub enum FootageSource {
    ...
    /// A **video file** decoded frame-by-frame via the `prism-media` FFmpeg CLI
    /// bridge. `fps` is the detected frame rate; `frame_count` is the total number
    /// of frames; `width`/`height` are the native resolution (preview scales to
    /// the comp's size automatically through the footage rasterizer).
    Video {
        path: PathBuf,
        fps: f64,
        frame_count: u64,
...
impl FootageSource {
    ...
    pub fn display(&self) -> String {
        match self {
            ...
            FootageSource::Video { path, .. } => path.display().to_string(),
        }
    }

    /// For Video: the source path and the frame index at comp time `t`.
    /// Returns `None` for Still/Sequence (handled via `path_for`).
    pub fn video_frame_at(&self, t: f32) -> Option<(&PathBuf, f64)> {
        if let FootageSource::Video { path, fps, frame_count, .. } = self {
            let duration_secs = *frame_count as f64 / fps.max(1.0);
            let t_secs = (t as f64).clamp(0.0, duration_secs);
            Some((path, t_secs))
        } else {
            None
        }
    ...
    /// Video sources use `video_frame_at` instead — `path_for` returns the
    /// video path itself (for use as a display key only).
    pub fn path_for(&self, seq: u32) -> PathBuf {
        match self {
            ...
            FootageSource::Video { path, .. } => path.clone(),
        }
    }
    ...
    pub fn len(&self) -> u32 {
        match self {
            ...
            FootageSource::Video { frame_count, .. } => (*frame_count).min(u32::MAX as u64) as u32,
        }
    }
...
/// Video file extensions recognized by `source_from_path`.
const VIDEO_EXTS: &[&str] = &["mp4", "mov", "mkv", "avi", "webm", "mts", "m2ts", "mxf", "flv"];

pub fn source_from_path(path: &Path) -> FootageSource {
    // Check video extensions before sequence detection (a video file like
    // "shot001.mp4" would otherwise be misidentified as an image sequence).
    let is_video = path
        .extension()
        .and_then(|e| e.to_str())
        ...
        .unwrap_or(false);
    if is_video {
        if let Ok(info) = prism_media::probe(path) {
            let frame_count = (info.duration_secs * info.fps).ceil() as u64;
            return FootageSource::Video {
        ...
        // If probe fails (ffprobe not installed), fall back to a still
        // so the layer at least appears in the comp.
    }
...
impl FrameCache {
    ...
    /// Get the decoded frame for a **video file** at `t_secs`, decoding (and
    /// caching) on first use. Cache key uses millisecond precision so each
    /// distinct frame gets its own slot without requiring fps knowledge.
    /// Returns `None` on decode failure (cached to avoid retrying).
    pub fn get_video(
        &mut self,
        path: &Path,
        t_secs: f64,
        ...
        let ms_key = (t_secs * 1000.0) as u64;
        let key = PathBuf::from(format!("video:{}:{}", path.display(), ms_key));
        if !self.entries.contains_key(&key) {
            let decoded = prism_media::decode_frame_at(path, t_secs, Some((1280, 720)))
                .ok()
                .map(|vf| rgba8_to_decoded_frame(&vf.rgba, vf.width, vf.height, alpha));
            if decoded.is_none() {
                log::warn!("video frame decode failed: {} t={:.3}s", path.display(), t_secs);
            }
            self.insert(key.clone(), decoded);
        } else {
            self.touch(&key);
        }
...
/// Convert a raw sRGB RGBA8 buffer (w×h×4 bytes) to a `DecodedFrame` in
/// linear-light straight RGBA. Shared by both `decode_to_linear` (image files)
/// and `FrameCache::get_video` (video frames from prism-media).
fn rgba8_to_decoded_frame(rgba8: &[u8], w: u32, h: u32, alpha: AlphaMode) -> DecodedFrame {
    ...
    DecodedFrame { width: w, height: h, pixels }
}
...
fn decode_to_linear(path: &Path, alpha: AlphaMode) -> Option<DecodedFrame> {
    ...
    Some(rgba8_to_decoded_frame(&loaded.rgba8, w, h, alpha))
}
