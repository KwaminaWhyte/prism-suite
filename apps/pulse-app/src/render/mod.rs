...
// Per-frame timing / path / range helpers, re-exported so a *second* front-end
// (the GPUI host) can drive its own frame-by-frame export loop — rendering on a
// background thread and reporting progress per frame — while reusing the exact
// framing, numbering, and work-area math the egui app's `export_sequence_in_project`
// uses internally. Purely additive: the egui binary still calls
// `export_sequence_in_project` and is unaffected.
pub use export::{frame_count, frame_path, frame_range, frame_time, ExportSummary};
#[cfg(test)]
pub use export::export_sequence;
use passes::{
    apply_adjustment, apply_distort, apply_dof, apply_key, apply_masks, apply_puppet_warp,
    apply_spatial, apply_stylize, apply_track_matte, composite_footage, composite_generate,
...
pub fn render_preview_frame(
    ...
    // Render at the comp's NATIVE resolution, then box-downsample to the capped
    // preview size. Layer geometry (positions, mask vertices, text size, …) is in
    // absolute comp pixels mapped from the comp center, so merely shrinking
    // `comp.width/height` would NOT scale the layout — it would center-crop it
    // (the title, positioned above the comp center, would fall off the top). A
    // full-res render + uniform downscale guarantees the preview is exactly the
    // exported frame, just smaller. Downsampled results are frame-cached upstream
    // (`CanvasHost`), so the native render only runs on a cache miss.
    // Interactive playback can't afford the comp's full motion-blur sample count
    // (the default demo uses 16 — a 16× per-blurred-layer cost spike). Clamp the
    // sample count for the live preview only; export/render-queue use the comp's
    // real value. Clone is paid only when a clamp is actually needed.
    let frame = if target.motion_blur.enabled && target.motion_blur.samples > PREVIEW_MB_SAMPLES_CAP {
        let clamped: Vec<Comp> = comps
            .iter()
            .map(|c| {
                if c.id == id {
                    let mut c = c.clone();
                    c.motion_blur.samples = PREVIEW_MB_SAMPLES_CAP;
                    c
                } else {
                    c.clone()
                }
            ...
            .collect();
        render_frame_in_project(&clamped, id, t, cache)
    } else {
        render_frame_in_project(comps, id, t, cache)
    };
    downscale_frame(frame, pw, ph)
}

/// Motion-blur sub-frame samples used for the **interactive preview** (export
/// uses the comp's real `motion_blur.samples`). Caps the per-frame cost spike of
/// high sample counts during scrub/playback while keeping the blur readable.
const PREVIEW_MB_SAMPLES_CAP: u32 = 4;

/// Area-average box downsample of an RGBA8 [`Frame`] to `(pw, ph)`. Returns the
/// frame untouched when it is already at the target size (the native-resolution
/// preview path). Averaging is done in straight (non-premultiplied) RGBA — the
/// composite is effectively opaque over the comp and this is a preview-only
/// resample, so edge fringing is negligible.
fn downscale_frame(src: Frame, pw: u32, ph: u32) -> Frame {
    let pw = pw.max(1);
    let ph = ph.max(1);
    ...
    for oy in 0..ph {
        let y0 = (oy as u64 * sh as u64 / ph as u64) as u32;
        let y1 = (((oy + 1) as u64 * sh as u64 / ph as u64) as u32)
            .max(y0 + 1)
            .min(sh);
        for ox in 0..pw {
            let x0 = (ox as u64 * sw as u64 / pw as u64) as u32;
            let x1 = (((ox + 1) as u64 * sw as u64 / pw as u64) as u32)
                .max(x0 + 1)
                .min(sw);
            let (mut r, mut g, mut b, mut a, mut n) = (0u32, 0u32, 0u32, 0u32, 0u32);
            for yy in y0..y1 {
                let row = (yy * sw) as usize * 4;
                for xx in x0..x1 {
                    let i = row + xx as usize * 4;
                    r += src.pixels[i] as u32;
                    g += src.pixels[i + 1] as u32;
                    b += src.pixels[i + 2] as u32;
                    a += src.pixels[i + 3] as u32;
                    n += 1;
                }
            }
            let n = n.max(1);
            let o = (oy * pw + ox) as usize * 4;
            out[o] = (r / n) as u8;
            out[o + 1] = (g / n) as u8;
            out[o + 2] = (b / n) as u8;
            out[o + 3] = (a / n) as u8;
...
pub(crate) fn render_comp(comp: &Comp, t: f32, cache: &mut FrameCache, ctx: RenderCtx) -> Frame {
    ...
    // Solo logic: if any layer has solo=true, only solo layers (and their parent
    // chains) render.
    let any_solo = comp.layers.iter().any(|l| l.solo);
    let should_render = |idx: usize| -> bool {
        if !any_solo {
            return true;
        }
        let mut cur = idx;
        let mut visited = vec![false; comp.layers.len()];
        loop {
            if cur >= comp.layers.len() || visited[cur] { break; }
            visited[cur] = true;
            if comp.layers[cur].solo { return true; }
            match comp.layers[cur].parent {
                Some(p) if p < comp.layers.len() && p != cur => cur = p,
                _ => break,
            }
        }
        false
    ...
    for i in comp.draw_order(t) {
        ...
        match layer.kind {
            // Null, Guide and ExpressionControl draw nothing — reference/control layers only.
            LayerKind::Null | LayerKind::Guide | LayerKind::ExpressionControl => {}
...
fn finish_layer(
    ...
    // Puppet warp: IDW displacement from pins (no-op with < 3 pins).
    if !layer.puppet_pins.is_empty() {
        apply_puppet_warp(layer_buf, geom, world, layer);
    }
