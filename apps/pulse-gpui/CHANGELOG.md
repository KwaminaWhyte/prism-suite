...
## [Unreleased] - 2026-06-19 (Batch 5)

### Fixed

- **Preview was tiny in the middle of the window.** The preview was drawn at its native capped pixel size (640×360) regardless of window size, so it floated small in a large work area. Now the preview is scaled to **fit the center work area** (window minus left strip / right dock / top toolbar / bottom timeline, with a small margin), preserving the comp aspect. The gizmo and pointer→comp mapping derive from the painted image bounds, so they stay aligned at any display scale.
- **Laggy playback (motion-blur cost).** Interactive playback re-composited the comp with the comp's full motion-blur sample count (the demo uses 16 → a 16×-per-blurred-layer spike) on every cache miss. The live preview now clamps motion-blur to `PREVIEW_MB_SAMPLES_CAP` (4) sub-frames; export / render-queue still use the comp's real sample count. Combined with the re-enabled frame cache, playback is far smoother.
- **Preview showed a center-crop of the comp, not the full frame.** The comp appeared zoomed/cropped — only a middle band was visible and the title (positioned above the comp center) was sliced off the top. Cause: `render_preview_frame` shrank only `comp.width/height` to the capped preview size while layer geometry stays in absolute comp pixels mapped from the comp center, so the layout was center-cropped at 1:1 instead of scaled down. Fixed: render at the comp's **native** resolution and box-downsample to the preview size (`downscale_frame`), so the preview is exactly the exported frame, just smaller. Regression-tested (`preview_is_downscaled_full_frame_not_center_crop`). Also re-enabled frame caching during playback (the real RAM bug was the sprite-atlas leak, now fixed; the LRU-capped cache keeps playback smooth since the native render only runs on a cache miss).
- **Right dock pushed off-window.** On a narrow window the right dock (Transform/Camera/Effects/Expressions) was shoved past the right edge and clipped (values, steppers, panel titles cut off). Cause: the center preview column was `flex_1` but its fixed 640px preview gave it a content-driven min-width, so it refused to shrink and forced the workspace row wider than the window, pushing the fixed-width dock off-screen. Fixed: center column gets `min_w(0)` + `overflow_hidden` (so it shrinks and clips the preview) and the dock gets `flex_shrink_0` (so it always keeps its width). Matches the reel/contour/pigment shell pattern.
- **Playback RAM leak (real cause).** RAM climbed from ~500 MB to multiple GB and never released during playback. Root cause was **not** the frame cache: gpui's sprite atlas only frees an image's GPU tile via an explicit `Window::drop_image`, and the host produces a brand-new `RenderImage` (new image id) every playback frame — so every frame leaked one atlas tile, unbounded. Fixed by holding the previously-painted image (`Pulse::last_image`) and calling `window.drop_image(prev)` whenever a new-id frame replaces it, bounding the atlas to ~one preview frame. The earlier frame-cache change stands as a secondary bound. This also smooths playback (no growing-atlas thrash).
- **Timeline not scrollable to the bottom.** The keyframe lanes shared one non-scrolling `flex_col` with the ruler inside the fixed-height timeline, so with many layers the bottom rows (e.g. "Solid 1") were clipped and unreachable. Fixed: the ruler stays pinned and the lanes now live in an `overflow_y_scroll` container; `TIMELINE_H` raised 180→240 so a few layers show without scrolling.

### Added (Batch 5)

- **Expanded expression library** (`comp/expr.rs`, additive). The per-property `rhai` engine gains the core After-Effects expression API beyond the original `wiggle`/`linear`/`clamp`:
  - **`loopOut()` / `loopOut("pingpong")` / `loopIn()` / `loopIn("pingpong")`** — repeat the keyed animation past the last / before the first key (cycle or ping-pong), folding the loop math over the track's `[first,last]` span.
  - **`valueAtTime(t)`** — read the keyframed value at an arbitrary time.
  - **`ease(t,t0,t1,v0,v1)` / `easeIn` / `easeOut`** — Hermite smoothstep remap (the smooth twin of `linear`).
  - **`random()` / `random(max)` / `random(min,max)` / `seedRandom(seed)`** — deterministic per-`(layer,time)` random (reproducible per frame, NOT `Math.random`); `seedRandom` pins the draw.
  - **`degreesToRadians` / `radiansToDegrees`** and **`width` / `height`** in the evaluation scope.
  - Mechanism: `ExprCtx` gains `width`/`height`; `Track::sample_expr` snapshots the track's keys into a new `expr::TrackView` and calls the new `expr::eval_with_track`, primed via thread-locals (mirrors the existing `wiggle` seed pattern). 9 new unit tests.
- **Comp Motion Blur panel + per-layer toggle (GPUI).** The existing comp `MotionBlur` model (master switch + shutter angle / phase / sample count) was modelled but had no GPUI UI. Added a **Motion Blur** section to the no-layer-selected Properties view: Enable ON/off toggle + Shutter°, Phase°, Samples steppers. Added a per-layer **MB** toggle to the selected-layer Properties (with a "(comp MB off)" hint when the comp master switch is off). Actions: `SetMotionBlurEnabled`, `SetMotionBlurAngle`, `SetMotionBlurPhase`, `SetMotionBlurSamples`, `ToggleLayerMotionBlur` (all undoable, re-render).
- **Posterize stylize effect** (`StylizeEffect::Posterize { levels }`). Quantises each straight-colour channel to `levels` evenly-spaced steps (cel-shade / screen-print bands); un-/re-premultiplied so alpha is preserved. Registry entry + `Levels` param + egui/GPUI UI. Pure pass unit-tested.
- **Invert stylize effect** (`StylizeEffect::Invert { amount }`). Photographic negative (`1-c` per channel), blended by `amount`; alpha preserved. Registry + `Amount` param + UI. Unit-tested.
- **Threshold stylize effect** (`StylizeEffect::Threshold { level }`). Rec.709-luminance black-and-white cut-off (high-contrast graphic matte); alpha preserved. Registry + `Level` param + UI. Unit-tested.
- `StylizeEffect::defaults()` grew 3 → 6; the Stylize browser folder now lists 7 entries (Glow + 6). Registry-sync + stack-order tests updated.

### Fixed

- Test-only `Comp` literals in `render/tests.rs` (×5) and `comp/tests.rs` (×1) were missing the Batch-3 `hide_shy` field, breaking the `cargo test` build (the lib itself compiled). Added `hide_shy: false`. Pre-existing breakage, unrelated to Batch 5 features.

## [Unreleased] - 2026-06-19 (Batch 4)

### Fixed

- **Playback RAM growth.** Pressing Play made RAM climb continuously and never release. Cause: `CanvasHost::frame_cache` auto-inserted a fresh RGBA frame every playback tick, growing to `FRAME_CACHE_CAP` (512 ≈ ~470 MB at the 640px preview) and never freeing on stop. Fixed: the auto frame cache is now skipped while `playing` (playback frames stream through the background compositor thread and are dropped after display — zero growth); the cache only fills while scrubbing/paused. Cap also lowered 512→256 to bound the scrub cache to ~240 MB.
- **Render Queue button clip.** "Render All" button overflowed the right dock's right edge: the header title had `flex_1` and pushed the three trailing buttons off-screen with no wrap/shrink. Fixed: header row `.flex_wrap()` + `.gap_1()`, title wrapped in `flex_1().min_w(0)` so it shrinks, "Render All" `.flex_none()` so it never clips.

### Added

- **3D Lights.** `Action::AddLight`, `RemoveLight`, `UpdateLight` applied to `comp.lights`. Properties panel (no-layer-selected) shows Lights section below Camera: per-light intensity steppers and Remove button; "Add Light" buttons for Ambient / Point / Parallel / Spot. Light brightness propagates via `layer_light_factor()` (existing renderer).
- **Expression Controls panel.** `panels/expr_controls.rs` — lists all ExprControl layers with kind badge (SL/∠/☑/◉/↖), value display, −/+ steppers (numeric) or ON/off toggle (checkbox). "Add Control" buttons for each kind at bottom. Registered in `mod.rs` and inserted into right dock in `main.rs`.
- **Output Module Presets.** `render_queue` panel shows "Output Presets" section with `preset.name` and "Apply" button per preset. Default 3 presets set in `App::new()`.
- **Multi-comp render.** "All Comps" button in render_queue header dispatches `Action::AddAllCompsToQueue`, which iterates all comps and enqueues a job per comp.
- **Format chips.** Render queue shows a `RenderFormat::ALL` chip strip (H.264 / H.265 / ProRes / GIF) so all format variants are constructed and visible.
- **Live Output toggle.** `App::live_output_enabled` + `Action::ToggleLiveOutput`. "Live Out" button in toolbar (amber when active). Each render cycle writes the most-recent RGBA frame to `/tmp/prism-pulse-preview.rgba` via `CanvasHost::write_live_output()`.

### Fixed

- Eliminated all 13 dead-code warnings: `RenderFormat::ALL` used in render_queue format picker; `Tool::label()` called in tools panel; `ExprControlKind::ALL`/`label()`/`name()` consumed by expr_controls panel; `ExprControlValue` fields destructured in expr_controls value display and `step_value`; `ExprControl::kind` read for badge; `OutputPreset::name` read in render_queue; `App::can_undo`/`can_redo` used in toolbar undo/redo dimming; unused `live_output_frame` field removed.

### Added (Batch 3)

- **Puppet warp.** New `crates/pulse-app/src/comp/puppet.rs` with `PuppetPin` struct and `PinId` type alias. `PulseLayer` gains `puppet_pins: Vec<PuppetPin>` (`#[serde(default)]`). Render pass `apply_puppet_warp` (IDW displacement, no-op with <3 pins) runs in `finish_layer` before compositing. `Tool::Puppet` added to tools strip (N icon).
- **Solo / Shy layer flags.** `PulseLayer` gains `solo: bool` and `shy: bool` (`#[serde(default)]`). `Comp` gains `hide_shy: bool`. Solo logic in `render_comp` skips non-solo layers (walking parent chains). GPUI layers panel shows S/H buttons per row and a "Shy" header toggle. Egui layers panel shows solo/shy buttons and "Hide shy" checkbox. Actions: `ToggleSolo`, `ToggleShy`, `ToggleHideShy`.
- **Time Remap UI.** GPUI timeline lane shows teal ◇ icon when `layer.time_remap.is_active()`. GPUI properties panel shows "Time Remap" section with ON/off toggle and source-time readout when active. Actions: `SetTimeRemapEnabled`, `SetTimeRemapKey`.
- **Track Mattes UI.** GPUI layers panel shows "T" badge on matte-source layers and matte-mode label badge on layers with active mattes. Action: `SetLayerMatte`.
- **Null object label.** Egui layers panel shows "(null)" label for `LayerKind::Null` layers. GPUI tools strip gains an "N" button that emits `Action::AddNullLayer`.

## [Unreleased] - 2026-06-18

### Added

- **`StrokePos` enum (Inside / Center / Outside) on `StylizeEffect::Stroke`.**
  `position: StrokePos` field added to the `Stroke` variant (`#[serde(default)]`
  → `Outside`, so all existing `.pulse` files load and render unchanged).
  The stroke pass is refactored into `stroke_outside()`, `stroke_inside()` (erode
  inward from the alpha edge — checks proximity to transparent pixels), and a
  `Center` dispatcher that runs both at `width/2`. Completes After Effects parity
  for the Stroke effect position knob.
- **`RenderFormat` enum with per-format render dispatch.** Four formats:
  `Mp4H264` (default), `Mp4H265`, `ProResProxy`, `Gif`. `RenderJob` gains a
  `format` field; `Action::AddToRenderQueue` infers the format from the file
  extension chosen by the user (`.mp4` → H.264, `.mov` → ProRes, `.gif` → GIF;
  unknown → H.264). `Action::RenderAll` dispatches per format:
  - **H.264 / H.265** → existing `prism_media::encode_h264` path (unchanged).
  - **ProRes Proxy** → FFmpeg pipe (`prores_ks -profile:v 0`) with a graceful
    "install FFmpeg" error if the binary is absent.
  - **Animated GIF** → `image::codecs::gif::GifEncoder` with per-frame `Delay`
    computed from comp fps.
  The render queue panel now shows a color-coded **format badge** next to each row
  (blue for H.264, teal for H.265, purple for ProRes, amber for GIF).
- **RAM preview frame cache (BTreeMap + LRU).** `CanvasHost` now holds a
  `BTreeMap<u64, Vec<u8>>` frame cache capped at 512 frames with LRU eviction via
  `VecDeque<u64>`. Cache hits bypass the background render thread entirely (BGRA
  conversion only). `Action::ClearRamPreview` clears the cache. Timeline ruler draws
  a 2px tall green strip at the top for every cached frame (iterates
  `cached_frame_set()` keys).
- **Layer parenting UI.** Properties panel now shows a "Parent" row when a layer is
  selected: a "None" clear-parent button plus one button per other layer. Clicking
  emits `SetParent(child, parent)` or `ClearParent(child)`. `SetParent`/`ClearParent`
  already wired the comp model (`PulseLayer.parent`) in a prior pass; the UI row now
  exposes it.
- **Motion Blur effect variant.** `Effect::MotionBlur { samples, shutter_angle }`
  added to the per-pixel `Effect` enum (index 10 in `defaults()`). Per-pixel `apply`
  is a no-op (temporal blur is a render-path concern); the variant exposes the two
  parameters for the UI. Added to effect browser under Color Correction category.
  `effect_params.rs` getters/setters for `samples` and `shutter_angle`.
- **3D Camera UI.** Properties panel shows a Camera section (Position XYZ + FOV
  steppers) when no layer is selected. Emits `SetCameraPosition([f32; 3])` and
  `SetCameraFov(f32)`. Toolbar gains a "3D Cam" button that deselects all layers to
  reveal the camera section.
- **ROI preview crop.** `SetROI(Option<[f32;4]>)` action now wired into the render
  thread. The compositor crops the rendered frame to the ROI rect (x,y,w,h in comp
  pixel space) before sending the result to the UI. Parity 100%.
- **Anchor point tool.** `Tool::AnchorPoint` lets users reposition a layer's pivot
  without moving the layer. Writes to `anchor_x`/`anchor_y` tracks. Badge in tools
  strip.
- **Displacement map effect.** `DistortEffect::DisplacementMap` warps a layer using
  another layer's R/G channels as a vector field. Wired into effect browser and GPUI
  param rows.
- **Expression controls.** `LayerKind::ExpressionControl` with Slider, Angle,
  Checkbox, Color, Point kinds. Toolbar buttons to add Slider/Angle controls.
  ExpressionControl layers render nothing but expose controllable values.
- **ProRes/DNxHD export via FFmpeg.** "ProRes" toolbar button opens file dialog, pipes
  rendered RGBA frames to `ffmpeg -c:v prores_ks`. Falls back to writing an error file
  if FFmpeg not found.
- **Output module presets.** `OutputPreset` struct, `output_presets` Vec on App.
  "+ Preset" button saves current format. `LoadOutputPreset`/`DeleteOutputPreset`
  actions.
- **Guide layer.** `LayerKind::Guide` — composited as nothing, shown with blue badge
  in layers panel. "+ Guide" toolbar button.
- **GIF export.** `Action::ExportGif` encodes animation frames via
  `image::codecs::gif::GifEncoder`. Progress indicator in toolbar.
- **Stroke effect.** `StylizeEffect::Stroke { width, color }` — morphological
  alpha-dilation pass, composited underneath original.
- **Video footage import.** `FootageSource::Video` variant stores probed metadata
  (fps, frame_count, width, height). `FrameCache::get_video` decodes frames on demand
  via `prism_media::decode_frame_at(path, t_secs, scale)`. `Action::ImportVideoFootage`
  opens a file picker, probes with `prism_media::probe`, and adds a Footage layer.
  "+ Video" toolbar button. Missing ffmpeg degrades gracefully (logs, no panic).

### Changed

- Toolbar stripped to essentials: app name, comp name, undo/redo, import, export
  (PNG/MP4/GIF), audio toggle, comp settings, RAM preview. Removed add-layer buttons,
  markers, expression-control chips, ProRes button, save-preset — these belong in
  menus/panels.
- `pulse-gpui`: ~31 hardcoded `rgb()` panel colors replaced with `prism_ui::colors::*`
  tokens. Text-label buttons replaced with `Icon` equivalents.
- `pulse-gpui`: gpui pinned to stable `0.2.2` (crates.io). `prism_ui::init(cx)` called
  at startup. Rust 1.95.0 required.

### Fixed

- Preview no longer zooms in on play: dual preview-cap (1280px static / 320px playing)
  removed; single 640px cap used for all renders via background thread.
- Display dimensions no longer reset during playback.

### Added

- **GPUI host: graph editor + preview transform gizmo (Wave 7).** The two
  highest-value *direct-manipulation* parity gaps land together, both **reusing
  the engine's pure modules** (`pulse_app::graph` math + `pulse_app::gizmo`) — no
  fork; the egui `pulse` binary and the comp model are unchanged.
  - **Graph (value-curve) editor on the timeline.** A new **Graph** toggle on the
    timeline header swaps the keyframe lanes for an After-Effects-style
    **value-over-time** plot of the selected layer's animated transform
    properties: each property gets its own colour, a shared value axis, a
    per-second grid, and a playhead guide. **Dragging a keyframe dot** retimes (x)
    and revalues (y) it (`Action::MoveKeyframeXY`); **dragging a Bézier ease
    handle** reshapes the segment's ease — promoting a Linear/Hold segment to an
    editable `Interp::Ease` seeded at the straight diagonal so the conversion is
    value-neutral, then reshaping it (`Action::SetInterp`). These are the *same*
    `(out_x, out_y, in_x, in_y)` control points the engine's `Track::sample`
    already evaluates, so the reshaped easing is reflected **live in the preview**.
    **Property chips** in the header choose which curves to plot (none selected =
    every keyed property). It's a GPUI port of `pulse_app::graph::show` — the same
    coordinate maps, hit model, and drag math — pointer-driven through a gpui
    `canvas` + mouse listeners (`pulse-gpui/src/panels/graph.rs`).
  - **Preview layer-select + transform gizmo.** Clicking the preview now
    **selects** whichever layer's quad sits under the pointer
    (`App::layer_at_pointer`, topmost-first), then draws an on-canvas **transform
    gizmo** for the selected layer — a bounding box, four corner **scale** handles,
    a **rotate** knob + connector, and the **anchor** cross — overlaying the
    rendered pixels exactly (geometry from the engine's `GizmoGeom::build` through
    the layer's world matrix). Dragging a handle **moves / scales / rotates** the
    layer (or moves its anchor) and **keys** the changed transform at the playhead
    (`Action::GizmoKeys` → `track.set_key`, exactly like the properties panel —
    writing keyframes on an animated track, a constant key on a static one). All
    the drag math is the engine's pure `gizmo::{hit_test, drag, parent_matrix,
    screen_to_comp}` (parent-local deltas, so a parented layer drags correctly
    under its parent's transform). A bounds-recording `canvas` shares the preview
    image's fit so the pointer↔comp mapping matches the drawn pixels
    (`pulse-gpui/src/panels/preview_panel.rs`). Both interactions are **undoable**
    (Cmd+Z) and pause playback on grab. New `App` state +
    `Action::{ToggleGraph, ToggleGraphProp, ClearGraphProps, SetInterp,
    MoveKeyframeXY, GizmoKeys}` arms in `app_state.rs`, with unit tests for the
    graph interp/move actions, the gizmo drag, and layer pick (5 new; 22 total in
    `pulse-gpui`).
- **GPUI host: Export + work-area / loop region (Wave 6).** The GPUI host can now
  **produce output** and bound playback/render to a sub-range — closing the two
  highest-value parity gaps. Both reuse the existing engine — no fork; the egui
  `pulse` binary and the comp model are unchanged.
  - **File → Export (PNG sequence, background-threaded, with progress).** A new
    toolbar **Export…** action pops a folder picker, then renders the active comp
    to a **PNG image sequence on a background thread** (the window stays
    responsive throughout — the CPU compositor runs off the UI/event-loop thread).
    The worker (`pulse-gpui/src/export.rs`) drives the *same* render engine the
    egui exporter does: `render::render_frame_in_project` for the pixels, and the
    newly **re-exported** (purely additive) `render::{frame_count, frame_time,
    frame_range, frame_path}` for the chosen range's frame span, presentation
    times, and zero-padded `comp_NNNN.png` filenames — so a GPUI-host sequence is
    byte-identical to one the egui app writes. A shared `ExportProgress` handle
    the toolbar polls each frame shows a live **"Exporting N/M…"** readout with a
    progress fill, then a final **"Exported M frames → …"** / **"Export failed:
    …"** chip (click to dismiss). Export defaults to the After Effects-style
    range — the **work area** when trimmed, else the **full comp**.
    *(MP4 is intentionally deferred: the suite ships no video encoder yet — no
    `prism-media` crate, `prism-io` is stills-only — and the egui app it mirrors
    only writes PNG. The `OutputFormat` enum + threading/progress scaffold leave a
    clean seam for an MP4 muxer.)*
  - **Work area / loop region.** The active comp's **work area** (the engine's
    existing `Comp::work_area` / `clamped_work_area`) is now editable in the GPUI
    host: a tinted band + draggable **in/out handles** on the timeline ruler
    (mouse-down arms `App::wa_drag`, ruler drag emits `Action::SetWorkAreaStart` /
    `SetWorkAreaEnd`; **right-click resets** to the full timeline via
    `Action::ResetWorkArea`). Playback now **loops within the work area** (the
    play `tick` wraps at the out-point / before the in-point, mirroring the egui
    app's RAM-preview loop instead of looping the whole timeline), and the
    **playhead dims** when it sits outside a trimmed area. Work-area edits are
    undoable. The handles' x↔time mapping reuses the timeline's existing
    `track_bounds` / `time_for_x`.
  - **Tests.** New unit tests cover the work-area Actions + work-area-bounded play
    loop, the export default-range rule, `ExportState::fraction` clamping, and two
    **end-to-end export** tests that run the real worker against a tiny comp and
    assert it writes the exact full-range / work-area frame counts.
- **GPUI host: effects panel + undo/redo (Wave 5).** The GPUI host gains a real
  effects panel in the right dock and an app-side, snapshot-based undo/redo
  stack. Both reuse the existing engine (`pulse_app::comp::*`) — no fork; the
  egui `pulse` binary and the comp model are unchanged.
  - **Effects panel.** For the selected layer, the new `panels::effects` lists
    every effect across all six per-layer stacks (colour `effects`,
    `spatial_effects`, `distort_effects`, `stylize_effects`, `key_effects`, and
    the single `generate` slot) as labelled cards. Each card has a `×` **remove**
    button (`Action::RemoveEffect`) and editable scalar **stepper rows** (`−` /
    value / `+`, since gpui 0.2.2 has no slider) emitting
    `Action::SetEffectParam`; the preview re-composites live.
  - **Add from a browser.** A toggleable Effects & Presets **browser** renders
    the same searchable registry the egui app uses
    (`comp::filter_grouped` → `BrowserEntry::instantiate` → `NewEffect`),
    grouped by category. Clicking an entry emits `Action::AddEffect(entry)`,
    appending the instantiated effect to the layer's matching stack — mirroring
    the egui app's `add_browser_effect`.
  - **Scalar-param accessor.** A new pure `effect_params` module exposes each
    effect variant's editable scalar parameters as `(label, value, lo, hi, step)`
    rows with an in-place setter, with ranges/labels mirroring the egui app's
    per-effect slider rows. Covers all six stacks (the `[f32;2]` points surface
    as X/Y rows; integer counts round; colours / bools / mode-enums are out of
    scope for the stepper cut). Unit-tested against the engine enums.
  - **Undo / redo (parity-critical).** The host now keeps a lightweight,
    snapshot-based history (a capped stack of `Project` clones). `App::apply`
    snapshots the pre-edit document for every document-mutating action
    (layer visibility, transform edits, keyframe set/move, and the new effect
    add/remove/param edits), so each is undoable; a fresh edit clears the redo
    stack (linear history). New `Action::Undo` / `Action::Redo` are bound to
    **Cmd+Z / Cmd+Shift+Z** on the focused root and to **Undo / Redo** buttons in
    the toolbar (greyed when their stack is empty). An undo/redo that swaps the
    whole project re-renders, clamps the playhead, and drops a now-stale
    selection / in-flight keyframe drag. Snapshot undo lives app-side because
    `pulse-app` has no existing history model (confirmed); the comp model is
    untouched.

- **GPUI host: keyframe lanes on the timeline (Wave 4).** The bottom timeline's
  layer lanes are no longer name-only placeholders — each lane now draws its
  transform keyframes and the selected layer's keys are time-draggable, with the
  preview reflecting the interpolated value live as the playhead moves. Reuses
  the existing `comp::{Track, Keyframe}` engine (no fork); the egui `pulse`
  binary and the comp model are unchanged.
  - **Keyframe diamonds.** Every layer lane paints a teal **diamond** at each
    transform keyframe time — the de-duplicated union of all transform tracks'
    key times — positioned by a fractional offset within the track region so it
    aligns with the scrub ruler above. Drawn via a per-lane `canvas` overlay
    using GPUI `Path`/`paint_path`.
  - **Set a keyframe (stopwatch).** Each Properties row gains a per-property
    **stopwatch** toggle (filled `◆` when animated, hollow `◇` when not, like
    After Effects). Clicking it emits the new `Action::ToggleKeyframe(prop)`:
    the first click lays a key at the playhead (the live expression-resolved
    value) so the track starts animating; a second click clears the track back
    to static. With a property animated, editing its value at a new playhead
    time adds a key (the existing `Action::SetTransform` → `Track::set_key`).
  - **Drag a keyframe.** Pressing a diamond on the *selected* layer's lane arms
    a horizontal drag (`App::kf_drag`); lane-level pointer motion emits the new
    `Action::MoveKeyframe { prop, key_index, time }` → `Track::move_key`, which
    re-times the key (preserving its value), re-sorts if it crosses a neighbour,
    and keeps the in-flight drag tracking the same key across the re-sort. The
    host re-renders each step so the preview shows the new interpolation live.

- **GPUI front-end (Phase 1+2 starter): `pulse-gpui` binary** — a second,
  GPUI-based host that coexists with the egui `pulse` binary (no rip-out; the
  egui app's behavior is unchanged). It mirrors the architecture already proven
  in the sibling Pigment app's `pigment-gpui`:
  - **Crate split.** `pulse-app` now exposes a `lib` (`src/lib.rs`,
    `pub mod comp/render/preview/…`) so the GPUI host can reuse the **comp model
    + CPU software compositor** without forking the engine; `main.rs` becomes the
    thin egui entry point (`use pulse_app::app::PulseApp`). The egui binary still
    boots identically.
  - **CPU preview bridge (no wgpu).** Unlike Pigment's wgpu host, `canvas_host`
    calls `pulse_app::render::render_preview_frame` (the existing capped
    ~1280px CPU compositor with a footage `FrameCache`) to render the active
    comp at the current playhead, converts the resulting 8-bit sRGB **RGBA8 →
    BGRA8** (GPUI's `RenderImage` byte order), wraps it as a `RenderImage`, and
    dirty-caches it — re-rendering only on a scrub or comp edit.
  - **State choke point.** `app_state::App` owns the `Project`, the playhead
    `time`, the active `Tool`, the selected layer, and the host; every panel
    mutation flows through the single `App::apply(Action)` choke point (which
    marks the host dirty when the composited frame changes).
  - **Host shell.** The root `Render` view lays out a top toolbar (title, active
    tool, live time/frame readout, working *Start* / *+1f* transport buttons), a
    left tools strip (Select/Hand/Pen, wired to `SetTool`), the center **bridged
    preview**, a right dock, and a **bottom timeline placeholder strip** (per-
    layer lanes + transport readout bound to live comp state).
  - **One real dock panel.** The **Layers** panel lists the active comp's layers
    (top = topmost) with a visibility dot (toggle → re-renders), a color chip,
    name, and kind; clicking a row selects it. Both interactions round-trip
    through `App::apply` + `cx.notify()`, the working reference for porting
    further panels in parallel.

- **GPUI host: editable Transform properties panel.** Selecting a comp layer
  (click a row in the Layers panel) now reveals an editable **Properties** panel
  in the right dock for that layer's transform — **anchor X/Y, position X/Y,
  scale, rotation, opacity** — one **stepper row** per property (a `−` / value /
  `+` control, since gpui 0.2.2 has no native slider, matching the toolbar's
  transport buttons).
  - **Live edit at the playhead.** A stepper emits the new
    `Action::SetTransform(prop, value)`, which routes through `App::apply` →
    `layer.track_mut(prop).set_key(time, value)` — the **same write the egui
    properties slider performs**: it edits the keyframed value *at the current
    playhead time* (overwriting the key on this instant when one exists, or
    laying down the base constant key when the track is unkeyframed), marks the
    host dirty, and re-samples `render_preview_frame` so the preview updates live
    at the current time.
  - **Expression-resolved readout.** Each row shows `Comp::layer_value` (the
    expression-aware sample) so the displayed value matches what the preview
    renders, plus a small keyframe count; taps compound off the live value (re-
    read at click time), not a captured snapshot.
  - **No selection** shows a hint and the panel keeps its dock slot. The egui
    `pulse` binary and the comp model are unchanged (the panel reuses the
    existing `Prop` / `Track::set_key` engine, no fork).

- **GPUI host: live, scrubbable + playable preview transport.** The host's
  timeline is no longer a static placeholder — its preview is driven by a real
  transport that re-samples the CPU compositor:
  - **Scrub (click + drag).** The bottom timeline gains a **scrub ruler** that
    records its painted bounds (via a `canvas`) into a shared `App::track_bounds`
    cell, then maps a pointer x → comp time (`App::time_for_x`) and emits the new
    `Action::SetTime`, which clamps the playhead and marks the host dirty so the
    next frame re-renders `render_preview_frame` at the new time. Drag tracking is
    gated by an `App::scrubbing` flag; mouse-up / mouse-up-out end the drag.
  - **Play / pause.** A toolbar **Play/Pause** toggle emits `Action::TogglePlay`
    (and `Action::Pause`). The root view drives the loop entirely on the UI
    thread: while `app.playing`, each `render` calls `App::tick` — which advances
    `time` by the **real wall-clock delta** since the last tick (clamped to 0.1s,
    looping back to 0 at the comp end) and marks the host dirty — then calls
    `window.request_animation_frame()` to re-notify the view next frame. The loop
    self-sustains while playing and stops re-arming once paused (the CPU-host
    analog of egui's `ctx.request_repaint()` playback block). No audio.
  - **Playhead marker.** A thin teal vertical rule overlays the ruler + lanes at
    the current time fraction so the playhead position is always visible. Keyframe
    lane editing and the value-curve graph editor remain a later wave.

- **Spot + Parallel lights and distance falloff** (PLAN *Lights (M): point/spot/
  parallel/ambient; … cone*; the follow-ups flagged on the first Lights slice).
  The comp light system gains two new `LightKind` variants and an optional
  distance attenuation, all on the existing pure, testable shading
  (`light::illumination`) so they shade `accepts_lights` 3-D layers through the
  real compositor + motion-blur path + preview exactly like Ambient/Point.
  - **`LightKind::Spot`** — a cone-restricted source at `position` aimed along a
    new `direction`, with an inner-cone half-angle (`cone_angle`, degrees) and a
    soft-edge `penumbra` (degrees). Inside the inner cone it's full; across the
    penumbra ring it ramps smoothly (smoothstep) to zero; outside it's dark.
    Otherwise it's a Lambert `max(0, N·L)` diffuse like Point and honors distance
    falloff.
  - **`LightKind::Parallel`** — a directional (infinitely-far) source: all rays
    share the fixed world `direction`, **position-independent** with **no
    falloff**. Lambert diffuse with `L = −normalize(direction)`.
  - **Distance falloff** — a new `falloff` radius (comp px) on Point and Spot. The
    default `0.0` disables it (the light reaches infinitely far), so an existing
    Point renders **byte-identically**. A positive radius `r` attenuates the
    contribution by a smooth inverse-square-ish `(r / (r + d))²` over the
    surface→light distance `d` — `1.0` at the light, decaying with distance.
  - All four new `Light` fields (`direction`, `cone_angle`, `penumbra`, `falloff`)
    are `#[serde(default)]`, so every legacy `.pulse` file (and a hand-written
    Ambient/Point light) loads, fills the defaults, and renders **exactly as
    before** — the no-lights / no-opt-in identity `[1, 1, 1]` contract is
    untouched, and the byte-identical back-compat goldens stay green.
  - **Comp ▸ Lights** UI gains *Add spot* / *Add parallel* buttons and per-light
    editors for direction (Spot/Parallel), cone + penumbra (Spot), and falloff
    radius (Point/Spot).
  - New unit tests: spot inside-cone vs outside-cone vs penumbra ramp (+ hard edge
    at zero penumbra), parallel position-independence + back-facing + un-normalized
    direction, falloff attenuation with distance + the exact default-no-falloff =
    old-Point identity, the attenuation helper, determinism across all four kinds,
    and serde + legacy round-trips for the new fields.
