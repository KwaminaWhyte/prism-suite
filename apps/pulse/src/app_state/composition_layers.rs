//! Layer-management, footage, marker, split / time-stretch, light, comp
//! motion-blur and 3-D-camera/DoF actions for the active composition.
//!
//! Split out of `composition.rs` (which kept growing past the 1000-line limit):
//! `apply_composition` handles transport / keyframe / parent / precomp arms and
//! delegates everything else here via its `_` arm. These two files share the
//! same `App` impl and the domain types live in `composition.rs`.

use super::*;

impl App {
    /// The layer-management / footage / marker / split / light / comp-motion-blur
    /// / 3-D-camera tail of [`App::apply_composition`]. Reached via that match's
    /// `_` arm (never called directly with a non-layer action).
    pub(super) fn apply_composition_layers(&mut self, action: Action) {
        match action {
            Action::AddNullLayer => {
            ...
            _ => unreachable!("apply_composition_layers called with wrong action"),
        }
    }
}
