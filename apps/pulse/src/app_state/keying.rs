...
/// Brightness-independent **chromaticity** `(r/s, g/s)` where `s = r+g+b`. A
/// neutral gray maps to `(1/3, 1/3)`; a near-black pixel (`s≈0`) has no defined
/// hue and falls back to neutral so it stays OUT of the key. Comparing
/// chromaticity (not raw `r-l,g-l,b-l`, whose magnitude scales with brightness)
/// is what makes the chroma keyer treat a dim and a bright green alike.
fn chromaticity(rgb: [f32; 3]) -> (f32, f32) {
    let s = rgb[0] + rgb[1] + rgb[2];
    if s <= 1e-4 {
...
pub fn chroma_key_alpha(rgb: [f32; 3], cfg: KeyConfig) -> f32 {
    let (pr, pg) = chromaticity(rgb);
    let (kr, kg) = chromaticity(cfg.key_color);
    let d = ((pr - kr) * (pr - kr) + (pg - kg) * (pg - kg)).sqrt();
