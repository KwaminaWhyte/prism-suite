...
// The REGISTRY catalogue lives in `effect_browser_registry.rs` (extracted for
// the workspace size rule); re-export it so `filter` / the tests / and
// `comp`'s `REGISTRY as EFFECT_REGISTRY` re-export keep resolving.
pub use super::effect_browser_registry::REGISTRY;
...
mod tests {
    ...
    fn distort_family_is_findable() {
        ...
        for (name, queries) in [
            ...
            ("Displacement Map", ["displacement", "warp"]),
        ] {
            for q in queries {
                let hits = filter(q);
                assert!(
        ...
        // The Distort folder holds all five distorts on an empty query.
        let groups = filter_grouped("");
        let dist = groups
            .iter()
            .find(|(c, _)| *c == Category::Distort)
            .expect("Distort folder present");
        assert_eq!(dist.1.len(), 5, "five distorts in the Distort folder");
    }
    ...
    fn stylize_family_is_findable() {
        ...
        // The Stylize folder holds Glow (spatial stack) plus the six stylize-stack
        ...
        assert_eq!(stylize.1.len(), 7, "seven effects in the Stylize folder");
    }
    ...
    fn name_substring_filters() {
        ...
        // Every hit is in the Blur or Color Correction category (Motion Blur is
        // a color-stack effect whose name contains "blur").
        assert!(hits.iter().all(|h| {
            h.entry.category == Category::Blur || h.entry.category == Category::Color
        }));
        // The Blur & Sharpen family specifically is all present.
        assert!(hits.iter().any(|h| h.entry.category == Category::Blur));
    }
    ...
    fn grouped_drops_empty_categories() {
        // "blur" hits the Blur category (Gaussian/Box/Directional/Radial) and
        // also Color Correction because "Motion Blur" contains "blur".
        // Every returned group is non-empty (the definition of grouped_drops_empty).
        let groups = filter_grouped("blur");
        assert!(groups.iter().all(|(_, hits)| !hits.is_empty()),
            "every group must be non-empty");
        let blur_group = groups.iter().find(|(c, _)| *c == Category::Blur)
            .expect("Blur group present for 'blur' query");
        assert_eq!(blur_group.1.len(), 4, "four spatial blurs in Blur group");
    }
