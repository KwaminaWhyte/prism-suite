...
// The deterministic noise / cellular / hashing math lives in `generate_math.rs`
// (extracted for the workspace size rule); glob-import it so the generator impls
// above call `fbm` / `cellular` / `bias` / … exactly as when they were inline.
use super::generate_math::*;
