    ...
    pub fn sample_expr(&self, t: f32, default: f32, ctx: ExprCtx) -> f32 {
        ...
        match self.expression.as_deref() {
            Some(src) if !src.trim().is_empty() => {
                ...
                // Snapshot the track's keyframes so `valueAtTime`/`loopOut`/
                // `loopIn` can read the keyed value at any time, not just `t`.
                let view = expr::TrackView {
                    keys: self.keys.iter().map(|k| (k.t, k.value)).collect(),
                    default,
                };
                expr::eval_with_track(src, &ctx, Some(view)).unwrap_or(keyed)
            }
            _ => keyed,
        }
    }
