...
// The built-in stroke-font glyph table lives in `text_glyphs.rs` and the real
// outline-font layout machinery (TrueType layout + curve flattening + the
// straight source-over helper) in `text_outline.rs` — both extracted for the
// workspace size rule. Import what `TextLayer`'s methods call so the bodies
// above are byte-identical to when these lived inline.
use super::text_glyphs::glyph;
use super::text_outline::{layout_outline_line, over_straight, OutlineLayout, OutlineLine};
