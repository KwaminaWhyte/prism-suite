//! The per-layer **content compositing** passes extracted from `render/passes.rs`
//! (workspace size rule): solid/shape/text/footage/precomp rasterization, the
//! motion-blur snapshot averaging, the per-layer isolated-buffer driver, and the
//! generate-fill pass. The `apply_*` finishing passes (adjustment, matte, masks,
//! key, spatial, DoF, stylize, distort, puppet) live in the sibling
//! [`apply`](super::apply) module. Behaviour is unchanged — these are the exact
//! functions `passes.rs` defined inline, re-exported via `passes/mod.rs` so
//! `render/mod.rs` imports them as before.

use crate::comp::{
    apply_effects, apply_effects_masked, blend_masked, Affine2, Comp, DecodedFrame, GenerateEffect,
    PulseLayer,
};
use crate::render::{over, render_comp, Geom, Lin, RenderCtx};
...
pub(crate) fn decode_footage(
    cache: &mut crate::comp::FrameCache,
    layer: &PulseLayer,
    src_t: f32,
    comp_fps: f32,
) -> Option<DecodedFrame> {
    // Video footage: decode via prism-media using t_secs (not path cache).
    if let Some(src) = layer.footage.source.as_ref() {
        if let Some((path, t_secs)) = src.video_frame_at(src_t) {
            return cache.get_video(path, t_secs, layer.footage.alpha).cloned();
        }
    }
...
pub(crate) fn composite_precomp(
    out: &mut [Lin],
    geom: &Geom,
    world: Affine2,
    layer: &PulseLayer,
...
pub(crate) fn composite_motion_blur(
    out: &mut [Lin],
    geom: &Geom,
    comp: &Comp,
    idx: usize,
...
/// clear.
pub(crate) fn composite_generate(
    out: &mut [Lin],
    geom: &Geom,
    world: Affine2,
    layer: &PulseLayer,
