...
pub fn apply_effect(pixmap: &mut Pixmap, effect: &Effect, scale: f32) {
    ...
    match effect {
        ...
        Effect::Extrude3D { depth, angle_deg, color } => {
            // Render extrude: draw offset silhouette in `color` behind the artwork.
            let rad = angle_deg.to_radians();
            let ox = (depth * rad.cos() * scale).round() as i32;
            let oy = (depth * rad.sin() * scale).round() as i32;
            let shadow = shadow_layer(pixmap, *color, 1.0, 0.0);
            let mut out = Pixmap::new(pixmap.width(), pixmap.height())
                .unwrap_or_else(|| Pixmap::new(1, 1).unwrap());
            let pp = PixmapPaint {
                blend_mode: BlendMode::SourceOver,
        ...
        Effect::Glow { radius, color, opacity } => {
            // Outer glow: shadow layer at dx=0,dy=0 composited behind the artwork
            let shadow = shadow_layer(pixmap, *color, *opacity, radius * scale);
            let mut out = Pixmap::new(pixmap.width(), pixmap.height())
                .unwrap_or_else(|| Pixmap::new(1, 1).unwrap());
            let pp = PixmapPaint {
                blend_mode: BlendMode::SourceOver,
            ...
            out.draw_pixmap(0, 0, shadow.as_ref(), &pp, Transform::identity(), None);
            out.draw_pixmap(0, 0, pixmap.as_ref(), &pp, Transform::identity(), None);
            *pixmap = out;
        }
    }
