...
pub struct TextParams {
    ...
    /// Variable font axis values keyed by the 4-character OpenType tag (e.g.
    /// `"wght"`, `"wdth"`, `"slnt"`). The layout engine ignores these for now
    /// (ttf_parser does not apply axis blending at this layer), but they are
    /// stored for round-trip fidelity and future renderer support.
    /// `#[serde(default)]` keeps old `.contour` files back-compatible.
    #[serde(default)]
    pub font_axes: HashMap<String, f32>,
}
...
mod tests {
    ...
    fn glyph_outline_produces_geometry() {
        let params = TextParams {
            ...
            font_family: None, font_axes: Default::default(),
        };
