...
impl CanvasGpu {
    ...
    pub fn read_selection(&self, device: &wgpu::Device, queue: &wgpu::Queue) -> Option<Vec<f32>> {
        ...
        slice.map_async(wgpu::MapMode::Read, |r| {
            flog(&format!("[sel.readback] map_async callback: {:?}", r));
        });
        let poll_result = device.poll(wgpu::PollType::Wait {
            submission_index: None,
            timeout: None,
        });
        flog(&format!("[sel.readback] poll result: {:?}", poll_result));
        poll_result.ok()?;
        ...
        drop(data);
        buf.unmap();
        flog(&format!("[sel.readback] read {} pixels, first={:.3}", out.len(), out.first().copied().unwrap_or(0.0)));
        Some(out)
    }
