...
struct FParams {
    ...
    // 33 field blur (Blur Gallery — multi-pin variable blur: per-pixel radius
    //    interpolated between pins by inverse-distance weighting, then a local
    //    Gaussian; pins ride in the cr0/cr1/cr2 overflow slots).
    kind: u32,
    _p0: u32,
    _p1: u32,
    _p2: u32,
    ...
    // Field Blur (kind 33) reuses these as up to three pins (xy = uv position,
    // z = blur amount in px, cr0.w = pin count). All zero for every other kind
    // (a no-op), and all-zero here is identity.
    cr0: vec4<f32>,
    cr1: vec4<f32>,
    cr2: vec4<f32>,
};
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    if p.kind == 1u {
        ...
        // ---- Field Blur (kind 33): a multi-pin variable blur. The per-pixel
        // blur radius is interpolated between the pins by inverse-distance-squared
        // (IDW, power 2) weighting, then a local 2D Gaussian of that radius runs
        // at the pixel — the same variable-radius kernel as Tilt-Shift / Iris but
        // the radius field comes from pin interpolation. A single pin → uniform
        // blur; no pins / all-zero amounts → identity. The pins ride in the `cr`
        // overflow slots (reused from Camera Raw): up to three pins as
        //   cr0 = (p0.x_uv, p0.y_uv, p0.amount_px, pin_count)
        //   cr1 = (p1.x_uv, p1.y_uv, p1.amount_px, _)
        //   cr2 = (p2.x_uv, p2.y_uv, p2.amount_px, _)
        // positions in uv (0..1); the IDW runs in pixel space (matching the CPU
        // reference `field_blur_radius_at`, incl. the 1e-3 squared-distance floor).
        let dim = 1.0 / p.texel;                 // (w, h) in px
        let px = in.uv * dim;                    // this pixel in px
        let count = i32(p.cr0.w);
        var wsum = 0.0;
        var asum = 0.0;
        for (var k = 0; k < count; k = k + 1) {
            ...
            if k == 1 { pin = p.cr1; }
            if k == 2 { pin = p.cr2; }
            let pp = pin.xy * dim;               // pin pos in px
            let off = px - pp;
            let d2 = max(dot(off, off), 1e-3);
            let w = 1.0 / d2;
            wsum = wsum + w;
            asum = asum + w * pin.z;
        }
        var rad = 0.0;
        if wsum > 0.0 { rad = asum / wsum; }
        let r = i32(clamp(rad, 0.0, 64.0));
        if r <= 0 {
            return textureSample(input, samp, in.uv);
        }
        ...
        for (var j = -r; j <= r; j = j + 1) {
            for (var i = -r; i <= r; i = i + 1) {
                let g = exp(-0.5 * f32(i * i + j * j) / (sigma * sigma));
                sum = sum + textureSample(
                    input, samp,
                    in.uv + vec2<f32>(f32(i), f32(j)) * p.texel,
                ) * g;
