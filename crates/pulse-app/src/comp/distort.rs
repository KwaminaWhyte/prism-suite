...
pub enum DistortEffect {
    ...
    /// **Displacement Map**: warp the layer using another layer's pixels as a
    /// vector field. The R channel drives X displacement, G drives Y.
    /// `map_layer` is the index of the source layer in the comp's layer array.
    /// `scale_x` and `scale_y` are displacement magnitude in normalised units
    /// (1.0 = ± full-buffer-width shift).
    DisplacementMap {
        map_layer: usize,
        scale_x: f32,
        scale_y: f32,
...
impl DistortEffect {
    ...
    pub fn label(&self) -> &'static str {
        match self {
            ...
            DistortEffect::DisplacementMap { .. } => "Displacement Map",
        }
    }
    ...
    pub fn defaults() -> [DistortEffect; 5] {
        [
    ...
    pub fn apply(&self, buf: &mut [[f32; 4]], width: usize, height: usize) {
        ...
        match *self {
            ...
            // DisplacementMap requires a separate map buffer; skip in single-buf path.
            DistortEffect::DisplacementMap { .. } => {}
        }
    }
}

/// Apply a displacement-map warp: use `map_buf` R/G channels to offset pixels
/// of `buf`. `scale_x`/`scale_y` are displacement in normalised 0..1 units.
pub fn apply_displacement_map(
    ...
    for y in 0..height {
        for x in 0..width {
            let map_px = map_buf.get(y * width + x).copied().unwrap_or([0.5; 4]);
            let dx = (map_px[0] - 0.5) * 2.0 * scale_x * width as f32;
            let dy = (map_px[1] - 0.5) * 2.0 * scale_y * height as f32;
            let sx = x as f32 + dx + 0.5;
            let sy = y as f32 + dy + 0.5;
            buf[y * width + x] = sample_bilinear(&src, width, height, sx, sy);
        }
    }
}
