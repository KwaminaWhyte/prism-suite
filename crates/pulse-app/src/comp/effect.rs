...
pub enum LayerKind {
    ...
    Solid,
    Null,
    /// Guide layer — visible in the layer panel but skipped during compositing.
    /// Helps artists align elements without affecting rendered output.
    Guide,
    Adjustment,
    Shape,
    Text,
    Footage,
    Precomp,
    /// Expression control layer — exposes a single controllable value (slider,
    /// angle, checkbox, color, or point) that expressions in other layers can
    /// reference. Never composited into the output.
    ExpressionControl,
}
...
impl LayerKind {
    ...
    pub fn label(self) -> &'static str {
        match self {
            ...
            LayerKind::ExpressionControl => "Expression Control",
        }
    }

    /// Whether a layer of this kind draws its own pixels. Null, Guide, and
    /// Adjustment draw nothing of their own.
    pub fn draws_own_pixels(self) -> bool {
        ...
        // Null, Guide, Adjustment, ExpressionControl draw nothing.
    }
}
...
pub enum Effect {
    ...
    /// Motion Blur (per-layer effect variant) — a UI/pipeline hint that temporal
    /// multi-sample motion blur should be applied to this layer. The per-pixel
    /// `apply` is a no-op (motion blur is a temporal effect requiring multiple
    /// render samples; the actual multi-frame sampling is handled at layer render
    /// time when the comp-level `MotionBlur` settings are active). Adding this
    /// variant to a layer's effect stack marks it for the render path and exposes
    /// `samples` / `shutter_angle` in the properties panel.
    MotionBlur {
        /// Number of sub-frame samples for the temporal accumulation (1 = no blur).
        samples: u32,
        /// Simulated shutter opening angle in degrees (0–360; 180 = one half-frame).
        shutter_angle: f32,
    },
}
...
impl Effect {
    ...
    pub fn apply(&self, rgba: [f32; 4]) -> [f32; 4] {
        ...
        let out = match *self {
            ...
            // Motion Blur is a temporal effect; at the per-pixel level it is a
            // no-op — the render path handles the multi-frame accumulation.
            Effect::MotionBlur { .. } => [r, g, b],
        };
        [
            out[0].clamp(0.0, 1.0),
            out[1].clamp(0.0, 1.0),
