...
//!
//! Defaults mirror the egui app (`pulse_app::app::PulseApp::new`) so parity is
...
use pulse_app::gizmo::{self, GizmoGeom, Handle as GizmoHandle};
...
use pulse_app::render::RenderRange;
...
pub struct SubComp {
    pub name: String,
    pub layers: Vec<pulse_app::comp::PulseLayer>,
}
...
pub enum Action {
    ...
    // --- Batch 3: Track matte ---
    SetLayerMatte { layer_id: usize, mode: pulse_app::comp::MatteMode },
...
pub struct App {
    ...
    /// least one keyframe (mirrors `pulse_app::graph::GraphState::shown`).
...
/// engine's [`pulse_app::comp::Handle`], re-named to avoid clashing with the
/// gizmo's `Handle`.)
pub use pulse_app::comp::Handle as GizmoHandle2;
    ...
    pub fn apply(&mut self, action: Action) {
        ...
        match action {
            ...
            Action::ExportMp4(path) => {
                ...
                let comps = self.project.comps.clone();
                let comp_id = comp.id;
                let mut cache = pulse_app::comp::FrameCache::new();
                let mut frames: Vec<Vec<u8>> = Vec::with_capacity(total_frames as usize);
                for fi in 0..total_frames {
                    let t = fi as f32 / fps as f32;
                    let frame = pulse_app::render::render_frame_in_project(&comps, comp_id, t, &mut cache);
                ...
                match prism_media::encode_h264(frames.into_iter(), &params, &path) {
                    ...
                    Err(e) => {
                        ...
                        let mut cache2 = pulse_app::comp::FrameCache::new();
                        for fi in 0..total_frames {
                            let t = fi as f32 / fps as f32;
                            let frame = pulse_app::render::render_frame_in_project(&comps, comp_id, t, &mut cache2);
            ...
            Action::ExportGif(path) => {
                ...
                for fi in 0..total_frames {
                    let t = fi as f32 / fps;
                    let frame = pulse_app::render::render_preview_frame(&comps, comp_id, t, 640, &mut cache);
            ...
            Action::BuildRamPreview => {
                ...
                for fi in 0..total_frames {
                    let t = fi as f32 / fps;
                    let frame = pulse_app::render::render_preview_frame(&comps, comp_id, t, 1280, &mut cache);
            ...
            Action::PreCompose(indices, name) => {
                ...
                let placeholder = pulse_app::comp::PulseLayer::of_kind(
                    pulse_app::comp::LayerKind::Null,
                    name,
                    [0.5, 0.5, 0.5, 1.0],
                );
            ...
            Action::AddNullLayer => {
                let ci = self.active_comp_index();
                let null = pulse_app::comp::PulseLayer::of_kind(
                    pulse_app::comp::LayerKind::Null,
                    "Null 1",
                    [0.8, 0.8, 0.8, 1.0],
                );
            ...
            Action::AddGuideLayer => {
                ...
                let guide = pulse_app::comp::PulseLayer::of_kind(
                    pulse_app::comp::LayerKind::Guide,
                    &format!("Guide {n}"),
                    [0.2, 0.2, 0.8, 1.0],
                );
            ...
            Action::AddSolidLayer(rgba) => {
                ...
                let layer = pulse_app::comp::PulseLayer::of_kind(
                    pulse_app::comp::LayerKind::Solid,
                    "Solid 1",
                    color,
                );
            ...
            Action::AddAdjustmentLayer => {
                let ci = self.active_comp_index();
                let layer = pulse_app::comp::PulseLayer::of_kind(
                    pulse_app::comp::LayerKind::Adjustment,
                    "Adjustment 1",
                    [1.0, 1.0, 1.0, 1.0],
                );
                self.project.comps[ci].layers.push(layer);
                ...
                if let Some(path) = path {
                    match prism_media::probe(&path) {
                        Ok(info) => {
                            ...
                            let mut layer = pulse_app::comp::PulseLayer::of_kind(
                                pulse_app::comp::LayerKind::Footage,
                                &name,
                                [0.4, 0.4, 0.8, 1.0],
                            );
                            layer.footage.source = Some(pulse_app::comp::FootageSource::Video {
                                path: path.clone(),
                                fps: info.fps,
                        ...
                        Err(e) => {
                            ...
                            layer.footage.source =
                                Some(pulse_app::comp::FootageSource::still(path));
                            self.project.comps[ci].layers.push(layer);
                            self.selected_layer =
                                Some(self.project.comps[ci].layers.len() - 1);
                            self.host.mark_dirty();
            ...
            Action::AddCompMarker { time, label } => {
                let ci = self.active_comp_index();
                let mut m = pulse_app::comp::Marker::at(time);
                m.label = label;
            ...
            Action::AddDisplacementMap { layer_idx, map_layer, scale_x, scale_y } => {
                ...
                if let Some(layer) = self.project.comps[ci].layers.get_mut(layer_idx) {
                    layer.distort_effects.push(
                        pulse_app::comp::DistortEffect::DisplacementMap {
                            map_layer,
                            scale_x,
                            scale_y,
                        }
            ...
            Action::SetDisplaceScale { layer_idx, effect_idx, scale_x, scale_y } => {
                let ci = self.active_comp_index();
                if let Some(layer) = self.project.comps[ci].layers.get_mut(layer_idx) {
                    if let Some(pulse_app::comp::DistortEffect::DisplacementMap { scale_x: sx, scale_y: sy, .. })
            ...
            Action::AddExpressionControl(kind) => {
                ...
                let mut layer = pulse_app::comp::PulseLayer::of_kind(
                    pulse_app::comp::LayerKind::ExpressionControl,
                    name,
                    [0.3, 0.5, 0.9, 1.0],
                );
                    ...
                    if let Ok(ref mut child) = child {
                        if let Some(stdin) = child.stdin.take() {
                            ...
                            for fi in 0..frame_count {
                                let t = fi as f32 / fps;
                                let frame = pulse_app::render::render_preview_frame(
            ...
            Action::AddPuppetPin { layer_id, pos } => {
                ...
                if let Some(l) = self.project.comps[ci].layers.get_mut(layer_id) {
                    ...
                    l.puppet_pins.push(pulse_app::comp::PuppetPin { id, position: pos, is_stiff: false });
                    self.host.mark_dirty();
                }
            }
...
mod tests {
    use super::*;
    use pulse_app::comp::filter_grouped;
    ...
    fn entry(name: &str) -> pulse_app::comp::BrowserEntry {
        filter_grouped("")
            .into_iter()
    ...
    fn set_interp_promotes_segment_and_is_undoable() {
        ...
        app.apply(Action::SetInterp {
            prop: Prop::X,
            key_index: 0,
            interp: Interp::Ease(pulse_app::comp::Ease::EASY),
        });
