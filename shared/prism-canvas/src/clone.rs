...
impl CanvasGpu {
    ...
    pub fn snapshot_clone_source(
        ...
        {
            let has_layer = self.layers.contains_key(&id);
            {
                use std::io::Write as _;
                if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/Users/kwamina/Desktop/solo/ prism/clone_debug.log") {
                    let _ = writeln!(f, "[snapshot_clone_source] id={id:?} has_layer={has_layer} layer_ids={:?}", self.layers.keys().collect::<Vec<_>>());
                }
            }
